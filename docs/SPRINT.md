...
| S-108 | Keep Step 3 review cards aligned with saved effective year data after `Save and sync year`. See S-108 substeps. | apps/web/src/v2/OverviewPageV2.tsx, apps/web/src/v2/yearReview.ts, apps/web/src/api.ts, apps/web/src/v2/OverviewPageV2.test.tsx, apps/web/src/v2/yearReview.test.ts | A sync-save of a manual year patch immediately updates the Step 3 review card to the same effective values returned by `GET /v2/import/years/:year/data`, without waiting for a hard reload or showing stale VEETI numbers. | Packet `764984e8b3b70b1ebf1fa07dd26146c31da73d13` refreshed synced-year cache in `runSync()` and added a focused Step 3 regression proving the card flips to refreshed effective values after sync-save. | Stop if truthful card refresh requires a broader Overview data-loading rewrite beyond the current year-cache and sync paths. | READY |
