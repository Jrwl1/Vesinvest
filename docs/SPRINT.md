...
| S-36 | Close the highest-value desktop accessibility and release-polish gaps from the audit, then rerun the full desktop-quality gates. | apps/web/src/components/LanguageSwitcher.tsx, apps/web/src/v2/EnnustePageV2.tsx, apps/web/src/v2/AppShellV2.tsx, apps/web/src/v2/v2.css, apps/web/src/i18n/locales/, apps/web/src/v2/, e2e/ | The desktop app resolves the audited accessible-name and field-label issues, preserves keyboard clarity across the refreshed V2 flow, and passes final web/root quality gates with the hardening changes in place. | DONE: acceptance verified against commits `385d796`, `c76a439`, `1f28f92`, and `3eff528`; desktop language buttons and Forecast investment controls expose clear accessible names, refreshed V2 focus-visible states use stronger rings plus focused-card treatment, and `pnpm --filter ./apps/web typecheck && pnpm --filter ./apps/web test -- src && pnpm lint && pnpm typecheck && pnpm test` passes. | Stop if acceptance would require mobile-specific redesign or unrelated SEO work outside the audited desktop trust/accessibility scope. | DONE |
...
- [x] Add explicit labels or labelled-by wiring for the investment editor inputs and selects that were unlabeled in the audit
  - files: apps/web/src/v2/EnnustePageV2.tsx, apps/web/src/v2/v2.css
  - run: pnpm --filter ./apps/web test -- src/v2 && pnpm --filter ./apps/web typecheck
  - evidence: commit:c76a439 | run:pnpm --filter ./apps/web test -- src/v2 && pnpm --filter ./apps/web typecheck -> PASS | files:apps/web/src/v2/EnnustePageV2.tsx | docs:N/A | status: clean
...
- [x] Recheck keyboard focus order and desktop interactive affordances after the hardening changes so the refreshed shell remains legible under keyboard-only use
  - files: apps/web/src/v2/, apps/web/src/App.css
  - run: pnpm --filter ./apps/web test -- src/v2
  - evidence: commit:1f28f92 | run:pnpm --filter ./apps/web test -- src/v2 -> PASS | files:apps/web/src/v2/v2.css | docs:N/A | status: clean

- [ ] Run final desktop-focused web and root quality gates after the trust, language, and accessibility fixes land
- [x] Run final desktop-focused web and root quality gates after the trust, language, and accessibility fixes land
  ...
  - evidence: commit:3eff528 | run:pnpm --filter ./apps/web typecheck && pnpm --filter ./apps/web test -- src && pnpm lint && pnpm typecheck && pnpm test -> PASS | files:apps/web/src/components/LanguageSwitcher.tsx | docs:N/A | status: clean
