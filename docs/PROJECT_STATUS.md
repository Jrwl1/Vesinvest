...
- `S-28` is `IN_PROGRESS`: Reports now has the refreshed shell, stronger selection/KPI hierarchy, explicit variants, publication-friendly provenance, tighter appendix blocks, clearer export/loading states, and aligned locale copy; only final regression proof remains before READY.
...
1. Continue execution from `S-28` substep 9 in `docs/SPRINT.md` using `DO` or explicit whole-sprint `RUNSPRINT`.
