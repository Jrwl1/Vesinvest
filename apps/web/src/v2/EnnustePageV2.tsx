...
export const EnnustePageV2: React.FC<Props> = ({
  ...
  }, [loadingList, scenarios, selectedScenarioId, loadScenario, scenario?.id]);
  ...
  const forecastFreshnessState = React.useMemo(
    ...
    [scenario, hasUnsavedChanges, isComputing],
  );
  ...
  const computedVersionLabel = React.useMemo(
    () =>
      scenario?.computedFromUpdatedAt
        ? formatScenarioUpdatedAt(scenario.computedFromUpdatedAt)
        : t('v2Forecast.reportStateMissing'),
    [scenario?.computedFromUpdatedAt, t],
  );
  ...
  const hasMissingDepreciationRules =
    depreciationRulesUnavailable &&
    draftInvestments.some((row) => row.amount > 0);
  ...
  const saveDepreciationRuleDraft = React.useCallback(
    ...
    [depreciationRuleDrafts, markScenarioAsNeedsRecompute, selectedScenarioId, t],
  );
  ...
  React.useEffect(() => {
    if (
      ...
      setActivePrimaryChart('cashflow');
      return;
    }
    if (activeWorkbench === 'revenue') {
      setActivePrimaryChart('price');
    }
  }, [activeWorkbench]);
  ...
  const investmentProgramSurface = (
    <ForecastInvestmentSurface
      t={t}
      depreciationRulesUnavailable={depreciationRulesUnavailable}
      investmentSummary={investmentSummary}
      forecastStateToneClass={forecastStateToneClass}
      forecastStateLabel={forecastStateLabel}
      investmentImpactSummary={investmentImpactSummary}
      hasInvestmentDepreciationErrors={hasInvestmentDepreciationErrors}
      invalidInvestmentDepreciationYears={invalidInvestmentDepreciationYears}
      onContinueToDepreciation={() => setActiveWorkbench('depreciation')}
      renderInvestmentProgramRows={renderInvestmentProgramRows}
      nearTermInvestmentRows={nearTermInvestmentRows}
      investmentProgramGroupOptions={investmentProgramGroupOptions}
      longRangeInvestmentGroups={longRangeInvestmentGroups}
      renderInvestmentEditorRows={renderInvestmentEditorRows}
      denseAnalystMode={denseAnalystMode}
      busy={busy}
      draftInvestmentsCount={draftInvestments.length}
      onCopyFirstInvestmentToAll={handleCopyFirstInvestmentToAll}
      onRepeatNearTermInvestmentTemplate={handleRepeatNearTermInvestmentTemplate}
      onClearAllInvestments={handleClearAllInvestments}
      allInvestmentRows={draftInvestments}
      formatEur={formatEur}
        ...
        <section className="v2-card v2-scenario-editor v2-forecast-editor">
          ...
          {!loadingScenario && !scenario ? (
            <div className="v2-empty-state v2-subcard">
              ...
              {scenarios.length === 0 && firstBaselineYear ? (
                <div className="v2-overview-year-summary-grid">
                  <div>
                    <span>{t('projection.v2.baselineYearLabel', 'Baseline year')}</span>
                    <strong>{firstBaselineYear.year}</strong>
                  ...
                  <div>
                    <span>{t('v2Forecast.baselineSourceLabel', 'Baseline source')}</span>
                    <strong>
                      {baselineSourceStatusLabel(firstBaselineYear.sourceStatus)}
                    </strong>
                  </div>
                </div>
              ) : (
            ...
            <>
                  ...
                  <div className="v2-section-header">
                    ...
                    <div className="v2-primary-chart-tabs">
                      <button
                        type="button"
                        className={`v2-btn ${
                          activePrimaryChart === 'cashflow' ? 'v2-btn-primary' : ''
                        }`}
                      ...
                      <button
                        type="button"
                        className={`v2-btn ${
                          activePrimaryChart === 'price' ? 'v2-btn-primary' : ''
                        }`}
              ...
              {activeWorkbench === 'depreciation' ? (
                <ForecastDepreciationSurface
                  t={t}
                  reportReadinessToneClass={reportReadinessToneClass}
                  reportReadinessLabel={reportReadinessLabel}
                  reportReadinessReason={reportReadinessReason}
                  reportReadinessHint={reportReadinessHint}
                  baselineDepreciationTotal={baselineDepreciationTotal}
                  newInvestmentDepreciationTotal={newInvestmentDepreciationTotal}
                  totalDepreciationEffect={totalDepreciationEffect}
                  requiredPriceToday={
                    scenario.requiredPriceTodayCombinedAnnualResult ??
                    scenario.requiredPriceTodayCombined ??
                    scenario.baselinePriceTodayCombined ??
                  ...
                  underfundingStartYear={
                    scenario.feeSufficiency.cumulativeCash.underfundingStartYear ??
                    null
                  }
                  peakGap={scenario.feeSufficiency.cumulativeCash.peakGap}
                  latestCashflow={latestCashflowPoint?.cashflow ?? null}
                  depreciationPreviewRows={depreciationPreviewRows}
                  unmappedInvestmentYears={unmappedInvestmentYears}
                  savedMappedInvestmentYearsCount={savedMappedInvestmentYearsCount}
                  plannedInvestmentYearCount={plannedInvestmentYears.length}
                  depreciationClassKeys={depreciationClassKeys}
                  draftInvestments={draftInvestments}
                  savedMappedDepreciationClassByYear={
                    savedMappedDepreciationClassByYear
                  }
                  ...
                  mappedDepreciationClassByYear={mappedDepreciationClassByYear}
                  handleAllocationDraftChange={handleAllocationDraftChange}
                  depreciationClassOptions={depreciationClassOptions}
                  applyCarryForwardMapping={applyCarryForwardMapping}
                  busy={busy}
                  canSaveClassAllocations={Boolean(selectedScenarioId)}
                  saveClassAllocations={() => void saveClassAllocations()}
                  loadingDepreciation={loadingDepreciation}
                  depreciationRuleDrafts={depreciationRuleDrafts}
                  getDepreciationRuleGroup={getDepreciationRuleGroup}
                  handleDepreciationRuleDraftChange={
                    handleDepreciationRuleDraftChange
                  }
                  saveDepreciationRuleDraft={(index) =>
                    void saveDepreciationRuleDraft(index)
                  }
                  formatEur={formatEur}
                  formatPrice={formatPrice}
