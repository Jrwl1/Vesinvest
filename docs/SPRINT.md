...
| S-115 | Clean mixed-language fallback and stale helper copy across wizard, Forecast, and Reports. See S-115 substeps. | apps/web/src/v2/**, apps/web/src/i18n/locales/*.json, apps/web/src/components/** | Primary Swedish/Finnish/English surfaces no longer leak fallback strings from other locales, and helper/empty-state copy reflects the actual screen state instead of stale onboarding text. | Packet `c8d8d00837ed32b5eb50c516fc7914c7a361297d` plus focused wizard/Forecast/Reports regressions, locale integrity, and clean web typecheck passed; row is ready for acceptance review. | Stop if locale parity requires a broader i18n migration outside the active V2 flow. | READY |
