...
Re-run the frontend HUMANAUDIT-derived rows `S-157..S-163` truthfully under the corrected row-driven OS contract because the earlier archive acceptance no longer matches current code reality.

## Recorded decisions (this sprint)

- `S-157..S-163` are active again on `2026-03-23`.
- Earlier archive acceptance for `S-157..S-163` is not trusted as current-code proof.
- `S-162` is an evidence gate and must be rerun, not treated as durable implementation proof.
...
- [ ] Replace the login hero and sign-in card copy with plain product/task language and remove repeated sign-in chrome across the card header
  ...
  - evidence: Reopened from archive on 2026-03-23 after code audit found acceptance not fully trustworthy as current-code proof.
