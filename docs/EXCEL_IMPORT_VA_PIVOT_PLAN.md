...
| 2 | Multiple budget blocks in same sheet: keep scanning all section headers (existing + row-76-style); parse each block; aggregate lines; stop at total/blank/next header. | Same adapter. | Unit test: two blocks, two header rows. |
| 3 | Revenue drivers from KVA totalt: parse rows 55–58 for Vatten/Avlopp prices and VAT; set `VaImportPreview.revenueDrivers`. | Adapter or sub-module. | Unit test: mock sheet with price rows. |
