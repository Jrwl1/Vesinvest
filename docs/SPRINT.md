...
- [ ] Run a final Finnish Kronoby UI/UX re-audit (`clear -> search -> connect -> explicit import -> review continue -> blocked-year fix/exclude -> baseline -> unlock`) and record the explicit sprint outcome in `docs/SETUP_WIZARD_UIUX_REAUDIT.md`
  ...
  - evidence: HARD BLOCKED: commit:55968d973fd755343b34c1f5ce075e6bae98d16e | run:N/A (manual browser audit) | files:docs/SETUP_WIZARD_UIUX_REAUDIT.md | reason: the rerun reached the wizard, but `POST /api/v2/import/clear` returned `401`, so the required clear-first flow could not proceed
