...
import type {
  ManualYearCompletionDto,
  ManualYearDocumentImportSourceLineDto,
  ManualYearWorkbookCandidateDto,
} from './dto/manual-year-completion.dto';
import type {
...
type ImportManualPatchContext = {
  ...
  evaluateBaselineReadiness(
    ...
    yearDataset:
      ...
      | null
      | undefined,
    summaryRows: ReturnType<
      ImportManualPatchContext['buildImportYearSummaryRows']
    >,
    tariffRevenueReason: 'missing_fixed_revenue' | 'mismatch' | null,
  ): {
  ...
  hydrateYearRowsWithTariffRevenueReadiness<
...
const CLEAR_IMPORT_CHALLENGE_TTL_MS = 5 * 60 * 1000;
const GENERIC_WORKBOOK_PREVIEW_FAILURE =
  'Workbook preview failed. Upload a valid .xlsx/.xlsm KVA workbook.';

function createClearImportToken(): string {
  return randomBytes(5).toString('hex').toUpperCase();
}

function hashClearImportToken(challengeId: string, confirmToken: string) {
  return createHash('sha256')
    .update(`${challengeId}:${confirmToken.trim().toUpperCase()}`)
    .digest('hex');
}
...
export function createV2ImportManualPatchSupport(
  ctx: ImportManualPatchContext,
) {
  const financialSupport = createV2ImportManualFinancialSupport(ctx);
  const purgeExpiredClearChallenges = () => {
    const now = new Date(Date.now());
    return ctx.prisma.$executeRaw`
      DELETE FROM "v2_import_clear_challenge"
      WHERE "expires_at" <= ${now}
         OR "used_at" IS NOT NULL
    `;
  };
  const findClearChallenge = async (
    challengeId: string,
  ): Promise<ClearImportChallenge | null> => {
    ...
    >`
      SELECT
        "org_id" AS "orgId",
        "user_id" AS "userId",
        "confirm_token_hash" AS "confirmTokenHash",
        "expires_at" AS "expiresAt",
        "used_at" AS "usedAt"
      FROM "v2_import_clear_challenge"
  ...
  return {
    ...
    async previewWorkbookImport(
      ...
      try {
        ...
        throw new BadRequestException(GENERIC_WORKBOOK_PREVIEW_FAILURE);
      }
      const availableYears =
        await ctx.veetiEffectiveDataService.getAvailableYears(orgId);
      ...
      const matchedYears = importedYears.filter((year) =>
        workbookYearSet.has(year),
      );
        ...
        matchedYears.map(async (year) => {
          ...
          return {
            ...
            rows: IMPORT_YEAR_SUMMARY_FIELDS.map(({ key, sourceField }) => {
              const currentRow =
                summaryRows.find((row) => row.sourceField === sourceField) ??
                null;
              ...
              return {
                ...
                suggestedAction:
                  currentRow?.effectiveSource === 'missing' &&
                  workbookValue != null
    ...
    async previewStatementImport(
      ...
      const veetiRow = (tilinpaatosDataset?.rawRows?.[0] ?? null) as Record<
        string,
        unknown
      > | null;
    ...
    async clearImportAndScenarios(
      ...
      const challenge = normalizedChallengeId
        ? await findClearChallenge(normalizedChallengeId)
        : null;
      const providedToken = (confirmToken ?? '').trim().toUpperCase();
      if (
        !challenge ||
        challenge.orgId !== orgId ||
        challenge.userId !== userId ||
        challenge.usedAt != null ||
        !providedToken ||
        !tokenHashesMatch(
          challenge.confirmTokenHash,
          hashClearImportToken(normalizedChallengeId, providedToken),
        )
      ) {
        throw new BadRequestException({
          message:
            'Confirmation token did not match. Database was not cleared.',
      ...
      if (new Date(challenge.expiresAt).getTime() <= Date.now()) {
        ...
        `;
        throw new BadRequestException({
          message: 'Confirmation token expired. Database was not cleared.',
          code: 'CLEAR_CONFIRMATION_EXPIRED',
        });
      }
      const consumedCount = await ctx.prisma.$executeRaw`
        UPDATE "v2_import_clear_challenge"
        SET "used_at" = NOW()
        WHERE "id" = ${normalizedChallengeId}
          AND "used_at" IS NULL
      `;
    ...
    async createImportClearChallenge(
      ...
      await purgeExpiredClearChallenges();
      const challengeId = randomUUID();
      const confirmToken = createClearImportToken();
      const expiresAtMs = Date.now() + CLEAR_IMPORT_CHALLENGE_TTL_MS;
      const expiresAt = new Date(expiresAtMs);
      const confirmTokenHash = hashClearImportToken(challengeId, confirmToken);
      await ctx.prisma.$executeRaw`
        INSERT INTO "v2_import_clear_challenge"
          ("id", "org_id", "user_id", "confirm_token_hash", "expires_at")
        VALUES
          (${challengeId}, ${orgId}, ${userId}, ${confirmTokenHash}, ${expiresAt})
      `;
      return {
        challengeId,
        confirmToken,
    ...
    async completeImportYearManually(
      ...
      if (!isAdmin) {
        throw new ForbiddenException(
          'Only admins can patch VEETI import years.',
        );
      }
      ...
      const existingYearDataset =
        typeof ctx.veetiEffectiveDataService.getYearDataset === 'function'
          ...
          : null;
      const now = new Date();
      const workbookCandidateRows =
        body.workbookImport?.candidateRows
      ...
      const workbookMatchedFields = [
        ...new Set(
          [
            ...
            ...workbookCandidateRows.map(
              (row: { sourceField: string }) => row.sourceField,
            ),
          ]
            .map((field: unknown) => String(field ?? '').trim())
            .filter((field: string) => field.length > 0),
      ...
      const createBaseProvenance = (
        ...
        kind === 'statement_import' && body.statementImport
          ...
          ? {
              ...
              sourceLines: (body.documentImport.sourceLines ?? []).map(
                (row: ManualYearDocumentImportSourceLineDto) => ({
      ...
      const buildSourceMeta = (
        provenance: OverrideProvenanceCore | OverrideProvenance,
      ) => ({
      ...
      const financialSourceMeta = buildSourceMeta(
        ctx.manualPatchSupport.mergeFinancialOverrideProvenance(
          ...
          workbookFinancialProvenance ??
          statementFinancialProvenance,
      );
      ...
      return {
        ...
        baselineMissingRequirements:
          afterRow?.baselineMissingRequirements ?? [],
