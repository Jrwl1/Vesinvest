...
- `S-28` is `IN_PROGRESS`: Reports now has the refreshed two-column list/preview shell and a clearer selection surface; KPI legibility, provenance, variants, and export details remain to be reworked.
...
1. Continue execution from `S-28` substep 3 in `docs/SPRINT.md` using `DO` or explicit whole-sprint `RUNSPRINT`.
