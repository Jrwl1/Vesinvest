...
async function computeProjectionAndExportPdf(page: Page): Promise<void> {
  ...
  const pdfResponsePromise = page.waitForResponse(
    (resp) =>
      /\/projections\/[^/]+\/export-pdf$/.test(new URL(resp.url()).pathname) && resp.request().method() === "GET",
    { timeout: 45_000 }
  );
...
test.describe("Ennuste smoke", () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    page.on("response", (resp) => {
      ...
      const expectedProjectionValidation =
        status === 400 &&
        (/\/projections\/[^/]+\/compute$/.test(pathname) || /\/projections\/compute-for-budget$/.test(pathname));
  ...
  test("full flow manual fallback: credential login -> KVA import -> projection compute -> PDF export", async ({
    page,
  }) => {
  ...
  test("manual setup flow: create baseline manually -> scenario + investments + assumptions -> compute", async ({
    page,
  }) => {
    await ensureScreenshotDir();
    await login(page);
