...
4. KVA Excel import + Talousarvio baseline are locked (Option A, `KVA totalt` single-source import, historical-only Talousarvio). Ennuste page is functionally complete per `docs/PROJECTION_UX_PLAN.md`, the trust hardening queue `S-31..S-36` is accepted, the initial wizard rollout `S-37..S-42` is landed, the corrective wizard queue `S-43..S-47` is accepted, and the current execution target in `docs/SPRINT.md` is the wizard UX-coherence queue (`S-48..S-52` today): make the shell truthful on direct routes and after clear/reset, keep human-facing year semantics strictly about imported workspace years, move the active step form above summary chrome, split step-2 importable years from repair-only years, demote duplicate summary surfaces, smooth the Forecast/Reports handoff, and end with a fresh UX consistency audit that explicitly says whether the sprint succeeded or stopped on a blocker.
