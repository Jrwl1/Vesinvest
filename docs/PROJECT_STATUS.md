...
- `S-27` is `IN_PROGRESS`: Forecast now separates planning inputs from outputs, uses a denser investment workspace, has clearer risk comparisons, and makes output/readiness state more legible; the remaining locale/regression substeps stay active.
...
1. Continue execution from `S-27` substep 8 in `docs/SPRINT.md` using `DO` or explicit whole-sprint `RUNSPRINT`.
