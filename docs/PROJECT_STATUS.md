...
- `S-28` is `IN_PROGRESS`: Reports now has the refreshed shell, clearer selection, stronger KPI hierarchy, explicit variants, a publication-friendly provenance summary, and tighter appendix blocks; export/loading states and locale alignment still remain.
...
1. Continue execution from `S-28` substep 7 in `docs/SPRINT.md` using `DO` or explicit whole-sprint `RUNSPRINT`.
