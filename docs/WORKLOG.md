...
- [15:45] DO: S-38 substep 2 - separate the visible step-1 and step-2 UI around `Yhdistä organisaatio` and `Tuo valitut vuodet` (sprint: S-38, links: 25ae80f)
...
- [22:34] REVIEW: S-181 accepted and marked DONE (findings: investment rows now persist depreciation rule snapshots, compute uses those item snapshots ahead of year-share compatibility, and report readiness follows the saved item-level rule state)
...
- [17:52] RUNSPRINT: final live audit now clears the shipped login, setup, forecast, depreciation-planning, investment-save, and reports handoff flow after removing the snapshot-save payload leak and updating the prod-style e2e to the current live journey (row: S-192, status: DONE)
