...
| S-111 | Auto-map common investment groups into `Poistosaannot` defaults without hiding manual control. See S-111 substeps. | apps/web/src/v2/**, apps/web/src/api.ts, apps/web/src/i18n/locales/*.json, apps/web/src/v2/EnnustePageV2.test.tsx, apps/api/src/v2/** | Common groups such as `network`, `plant`, and `meters` prefill the obvious `Poistosaanto` mapping when confidence is high, while ambiguous groups stay manual and report readiness remains truthful. | Packets `e11cf5a339af43daf6c5bb91141cec223917c981` and `37bc2512c285867760cec77b68b3724266232f7e` now prove exact-match-first auto mapping for obvious groups and explicit unmapped behavior for ambiguous categories. | Stop if deterministic aliasing would require unsafe guessing or unsupported cross-language fuzzy matching in the primary workflow. | READY |
