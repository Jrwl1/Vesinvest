...
- `S-28` is `IN_PROGRESS`: Reports now has the refreshed two-column shell, clearer list selection, and a stronger selected-report KPI summary; provenance, variants, and export details remain to be reworked.
...
1. Continue execution from `S-28` substep 4 in `docs/SPRINT.md` using `DO` or explicit whole-sprint `RUNSPRINT`.
