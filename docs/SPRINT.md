...
| S-159 | Simplify the step-2 import board hierarchy, chronology, and parked-year treatment. See S-159 substeps. | apps/web/src/v2/OverviewImportBoard.tsx, apps/web/src/v2/useOverviewSetupState.ts, apps/web/src/v2/overviewLabels.ts, apps/web/src/v2/v2.css, apps/web/src/v2/OverviewPageV2.test.tsx, apps/web/src/i18n/locales/*.json | Visible year cards read chronologically left-to-right inside each lane, the missing-data warning stays primary, `Sekundära huvudtal` is replaced with plain pricing/volume language, and parked/unselected years move behind a secondary disclosure that still scales when five or more good VEETI years are present. | Accepted via packets `5fc52e8e7db213ed1a959f5379669cff3be04964` and `fabb238a9f39defc71b125d535af3a564abd8acf`; required Overview/locale tests and web typecheck passed. | Stop if collapsing parked years or reordering cards breaks current imported-year state truth or makes five-year selection less recoverable than today. | DONE |
| S-160 | Repair step-2 inline row editing so it behaves like a local card correction flow, not a sticky slab. See S-160 substeps. | apps/web/src/v2/OverviewImportBoard.tsx, apps/web/src/v2/OverviewPageV2.tsx, apps/web/src/v2/useOverviewManualPatchEditor.ts, apps/web/src/v2/v2.css, apps/web/src/v2/OverviewPageV2.test.tsx | The full finance row is clickable for edit, save closes the step-2 editor, and after saving one card the user can reopen the same or another year normally without relying on tiny value-chip targets. | Accepted via packets `a86fb0692b33b6ec2ed75efe173386ed17005cae` and `e0b4ad95526929896a362216ef9b261160ec3ff4`; required Overview tests and web typecheck passed. | Stop if whole-row click or close-on-save behavior would require a broader draft-state model or would collapse current checkbox/action affordances into one ambiguous control surface. | DONE |
| S-161 | Make the result warning/signal truthful and understandable after inline financial edits. See S-161 substeps. | apps/web/src/v2/yearReview.ts, apps/web/src/v2/overviewManualForms.ts, apps/web/src/v2/OverviewImportBoard.tsx, apps/web/src/v2/OverviewPageV2.tsx, apps/web/src/i18n/locales/*.json, apps/web/src/v2/**/*.test.ts, apps/web/src/v2/**/*.test.tsx, apps/api/src/v2/v2.service.ts, apps/api/src/v2/dto/manual-year-completion.dto.ts, apps/api/src/v2/**/*.spec.ts | The visible result row and its warning text stay coherent after inline finance edits, `/ 0` jargon is removed, and if `TilikaudenYliJaama` remains a separately stored field the UI makes that truth explicit instead of implying silent derivation. | Accepted via packets `2d9c89c32cb484c87cfc97cfdf79516e01c0919d` and `d602b54e93d9211a862c0aedad521a20e33d44e1`; required web yearReview/Overview tests, API v2 service tests, and dual typecheck passed. | Stop if truthful result behavior requires a broader accounting-contract decision than the current manual-year patch seam can safely absorb. | DONE |
| S-162 | Close with focused wizard regressions and a connected-workspace re-audit that includes a five-year import case. See S-162 substeps. | apps/web/src/**, apps/api/src/v2/**, apps/api/src/veeti/**, apps/web/src/**/*.test.tsx, apps/api/src/**/*.spec.ts, docs/SETUP_WIZARD_UIUX_REAUDIT.md | Focused regressions plus a fresh connected-workspace live audit prove the login cleanup, VEETI search stabilization, support rail, chronological five-year board, parked-year disclosure, row-edit/save behavior, and result warning changes work without a new trust blocker, or record the blocker precisely. | Accepted via packet `854c47d7ec24283d83bb17a644386745628621df`, the full focused regression bundle, and the appended connected-workspace rerun covering reset -> connect -> import -> five-year recovery -> row edit/save -> baseline-context verification. | Stop if the re-audit still exposes a new trust or workflow gap outside `S-157..S-161`; record the blocker and stop there. | DONE |
...
- [x] Replace the current login pill/oval kicker and oversized four-row hero with one calmer text-first Vesipolku entry surface
  ...
  - evidence: packet:dd8059a6eddcdbe078995881f4f5372b59eed9e2 | run:pnpm --filter ./apps/web test -- src/components/LoginForm.test.tsx src/App.test.tsx src/i18n/locales/localeIntegrity.test.ts && pnpm --filter ./apps/web typecheck -> PASS | files:apps/web/src/App.css,apps/web/src/components/LoginForm.test.tsx,apps/web/src/components/LoginForm.tsx,apps/web/src/i18n/locales/en.json,apps/web/src/i18n/locales/fi.json,apps/web/src/i18n/locales/sv.json | docs:N/A | status: clean

- [x] Rewrite the left-rail login value points in FI/SV/EN so VEETI import and future-investment/water-price outcomes are literal and concrete
...
- [x] Make the visible step-2 and step-3 card `Tulos` recompute from visible finance-row edits after manual save
  ...
  - evidence: packet:724655d452a7a04b903f6c1157ab20a4de85c1ba | run:pnpm --filter ./apps/web test -- src/v2/OverviewPageV2.test.tsx src/v2/yearReview.test.ts && pnpm --filter ./apps/api test -- src/v2/v2.service.spec.ts && pnpm --filter ./apps/web typecheck && pnpm --filter ./apps/api typecheck -> PASS | files:apps/web/src/v2/OverviewPageV2.test.tsx,apps/web/src/v2/overviewManualForms.ts,apps/web/src/v2/useOverviewManualPatchEditor.ts | docs:N/A | status: clean

- [x] Keep the refreshed result truth consistent through cache refresh, review flow, and reload after the same expense edit
...
- [x] Remove the year-card result commentary block and replace the current result-warning phrasing with shorter literal status treatment only where still required
  ...
  - evidence: packet:7aae2e8f50228f96733fdaff7ea3d9d32cbf9931 | run:pnpm --filter ./apps/web test -- src/v2/OverviewPageV2.test.tsx src/i18n/locales/localeIntegrity.test.ts && pnpm --filter ./apps/web typecheck -> PASS | files:apps/web/src/i18n/locales/en.json,apps/web/src/i18n/locales/fi.json,apps/web/src/i18n/locales/sv.json,apps/web/src/v2/OverviewImportBoard.tsx,apps/web/src/v2/OverviewPageV2.test.tsx,apps/web/src/v2/OverviewPageV2.tsx,apps/web/src/v2/useOverviewSetupState.ts,apps/web/src/v2/v2.css | docs:N/A | status: clean

- [x] Fix compact secondary price and volume tiles so Finnish labels fit cleanly without broken bold overflow
...
- [x] Run the focused web/api regression bundle for login hierarchy, imported-year result behavior, and compact tile rendering
  ...
  - evidence: packet:cbf8d795dcab821597e7cc789e239dddde50b0a3 | run:pnpm --filter ./apps/web test -- src/components/LoginForm.test.tsx src/App.test.tsx src/v2/OverviewPageV2.test.tsx src/v2/yearReview.test.ts src/i18n/locales/localeIntegrity.test.ts && pnpm --filter ./apps/api test -- src/v2/v2.service.spec.ts && pnpm --filter ./apps/web typecheck && pnpm --filter ./apps/api typecheck -> PASS | files:docs/SETUP_WIZARD_UIUX_REAUDIT.md | docs:N/A | status: clean

- [x] Re-run a live audit covering login first paint, a real `194 000` expense edit, and compact tile fit, and record whether the queue succeeded or stopped on a blocker in `docs/SETUP_WIZARD_UIUX_REAUDIT.md`
  - files: apps/web/src/**, apps/api/src/v2/**, docs/SETUP_WIZARD_UIUX_REAUDIT.md
  - run: N/A (manual browser audit allowed)
  - evidence: packet:cbf8d795dcab821597e7cc789e239dddde50b0a3 | run:manual browser audit -> PASS (fresh unauthenticated open on `http://127.0.0.1:4173` showed the calmer login surface; logging into `admin@vesipolku.dev` and changing 2022 `Material och tjänster` from `10 525` to `194 000` dropped visible `Resultat` from `194 103` to `10 628`; returning to step 2 kept the updated result and the compact secondary tiles rendered cleanly) | files:docs/SETUP_WIZARD_UIUX_REAUDIT.md | docs:N/A | status: clean

### S-163 substeps

- [x] Force unauthenticated app entry to Finnish by default until the user explicitly switches language
  ...
  - evidence: packet:3066996ee4e0d84489dfd7a115878db8b3c93ff4 | run:pnpm --filter ./apps/web test -- src/App.test.tsx src/components/LoginForm.test.tsx src/components/LanguageSwitcher.test.tsx src/i18n/locales/localeIntegrity.test.ts && pnpm --filter ./apps/web typecheck -> PASS | files:apps/web/src/App.css,apps/web/src/components/LoginForm.test.tsx,apps/web/src/components/LoginForm.tsx,apps/web/src/i18n/locales/en.json,apps/web/src/i18n/locales/fi.json,apps/web/src/i18n/locales/sv.json | docs:N/A | status: clean

- [x] Add the existing in-app language selector to the login screen and replace FI/SV/EN login copy with one plain Finnish source meaning translated cleanly across all three locales
...
- [x] Render step-2 year cards chronologically within each lane and keep five-year import cases readable without newest-to-oldest reading order
  ...
  - run: pnpm --filter ./apps/web test -- src/v2/OverviewPageV2.test.tsx src/i18n/locales/localeIntegrity.test.ts && pnpm --filter ./apps/web typecheck
  - evidence: packet:5fc52e8e7db213ed1a959f5379669cff3be04964 | run:pnpm --filter ./apps/web test -- src/v2/OverviewPageV2.test.tsx src/i18n/locales/localeIntegrity.test.ts && pnpm --filter ./apps/web typecheck -> PASS | files:apps/web/src/v2/OverviewImportBoard.tsx,apps/web/src/v2/OverviewPageV2.test.tsx | docs:N/A | status: clean
...
- [x] Make the full step-2 finance row clickable for inline edit instead of limiting entry to the numeric value button
  ...
  - run: pnpm --filter ./apps/web test -- src/v2/OverviewPageV2.test.tsx && pnpm --filter ./apps/web typecheck
  - evidence: packet:a86fb06ec5d8a48371b32c03fe138a9d6276d56d | run:pnpm --filter ./apps/web test -- src/v2/OverviewPageV2.test.tsx && pnpm --filter ./apps/web typecheck -> PASS | files:apps/web/src/v2/OverviewImportBoard.tsx,apps/web/src/v2/OverviewPageV2.test.tsx,apps/web/src/v2/v2.css | docs:N/A | status: clean

- [x] Close the step-2 inline editor on save and keep same-card/other-card reopen behavior reliable without tiny-target recovery paths
  ...
  - run: pnpm --filter ./apps/web test -- src/v2/OverviewPageV2.test.tsx && pnpm --filter ./apps/web typecheck
  - evidence: packet:e0b4ad97f3eebc617a34185f32d6d457c3575c0d | run:pnpm --filter ./apps/web test -- src/v2/OverviewPageV2.test.tsx && pnpm --filter ./apps/web typecheck -> PASS | files:apps/web/src/v2/OverviewPageV2.test.tsx,apps/web/src/v2/useOverviewManualPatchEditor.ts | docs:N/A | status: clean

### S-161 substeps

- [x] Replace `/ 0` result jargon with plain warning copy and keep the visible result signal coherent after inline financial edits
  ...
  - evidence: packet:2d9c89ceef7b5f42ab4062c09e467eed1b7deca3 | run:pnpm --filter ./apps/web test -- src/v2/yearReview.test.ts src/v2/OverviewPageV2.test.tsx src/i18n/locales/localeIntegrity.test.ts && pnpm --filter ./apps/web typecheck -> PASS | files:apps/web/src/i18n/locales/en.json,apps/web/src/i18n/locales/fi.json,apps/web/src/i18n/locales/sv.json,apps/web/src/v2/OverviewImportBoard.tsx,apps/web/src/v2/OverviewPageV2.test.tsx,apps/web/src/v2/OverviewPageV2.tsx | docs:N/A | status: clean

- [x] If `TilikaudenYliJaama` remains a separately stored field, make that truth explicit across summary/build warning paths and cover it with web/api regression proof
  ...
  - evidence: packet:d602b54054cc6e56e78d8fa9137090f7706f970c | run:pnpm --filter ./apps/web test -- src/v2/yearReview.test.ts src/v2/OverviewPageV2.test.tsx && pnpm --filter ./apps/api test -- src/v2/v2.service.spec.ts && pnpm --filter ./apps/web typecheck && pnpm --filter ./apps/api typecheck -> PASS | files:apps/web/src/i18n/locales/en.json,apps/web/src/i18n/locales/fi.json,apps/web/src/i18n/locales/sv.json,apps/web/src/v2/OverviewImportBoard.tsx,apps/web/src/v2/OverviewPageV2.test.tsx,apps/web/src/v2/OverviewPageV2.tsx | docs:N/A | status: clean

### S-162 substeps

- [x] Run the focused login + wizard regression bundle for the new entry copy, support rail, chronology, parked-year, search, and row-edit behavior
  ...
  - evidence: packet:854c47d7ec24283d83bb17a644386745628621df | run:pnpm --filter ./apps/web test -- src/App.test.tsx src/components/LoginForm.test.tsx src/v2/AppShellV2.test.tsx src/v2/OverviewPageV2.test.tsx src/v2/yearReview.test.ts src/i18n/locales/localeIntegrity.test.ts && pnpm --filter ./apps/api test -- src/v2/v2.service.spec.ts && pnpm --filter ./apps/web typecheck && pnpm --filter ./apps/api typecheck -> PASS | files:docs/SETUP_WIZARD_UIUX_REAUDIT.md | docs:N/A | status: clean
...
- [ ] Re-run a connected-workspace live audit through VEETI connect, year import, five-year selection/recovery, row edit/save, and baseline context, and record whether the queue succeeded or stopped on a blocker in `docs/SETUP_WIZARD_UIUX_REAUDIT.md`
  ...
  - run: N/A (manual browser audit allowed)
  - evidence: packet:854c47d7ec24283d83bb17a644386745628621df | run:manual browser audit -> PASS (reset `timing.audit.b@dev.local`, searched VEETI org `1535`, recovered parked year `2015`, completed and imported `2016` to reach a five-year set, saved a `2024` row edit in place, and verified support-rail baseline context updates) | files:docs/SETUP_WIZARD_UIUX_REAUDIT.md | docs:N/A | status: clean
