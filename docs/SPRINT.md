...
| S-28 | Refresh Reports with stronger publication and provenance UX after Forecast is accepted. | apps/web/src/v2/ReportsPageV2.tsx, apps/web/src/v2/v2.css, apps/web/src/i18n/locales/, apps/web/src/api.ts, apps/api/src/v2/v2.service.ts | Reports adopt the refreshed layout, keep current variant/report snapshot behavior, and make provenance and included-section clarity explicit for both public and confidential outputs. | Evidence in progress: report shell/layout primitives landed; remaining Reports substeps still pending. | Stop if the report UX refresh requires changing the saved report snapshot contract instead of re-presenting existing report data. | IN_PROGRESS |
