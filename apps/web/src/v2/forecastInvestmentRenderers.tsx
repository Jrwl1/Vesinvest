...
export function renderForecastInvestmentProgramRows({
  ...
  return rows.map((row) => {
    ...
    const classLabel = resolveVesinvestGroupLabel(
      t,
      row.groupKey,
      row.category ?? null,
    );
    const rowHasPlannedInvestment = resolveInvestmentProgramTotal(row) > 0;
    ...
    const depreciationSummary =
      ...
      (rowHasPlannedInvestment
        ...
        : t('v2Vesinvest.none', 'None')) ??
      t('v2Vesinvest.none', 'None');
    const linkedDepreciationSummary =
      snapshotDepreciationSummary ??
      (rowHasPlannedInvestment
        ? depreciationRulesUnavailable
          ? t(
    ...
    return (
      <div key={`program-${rowKey}`} className="v2-investment-program-row">
        ...
        {isVesinvestLinked ? (
          renderReadonlyInvestmentCell(formatEur(row.waterAmount ?? 0), null)
        ) : (
          <input
            className="v2-input"
            type="number"
        ...
        {isVesinvestLinked ? (
          renderReadonlyInvestmentCell(
            formatEur(row.wastewaterAmount ?? 0),
            null,
          )
        ) : (
          <input
        ...
        {isVesinvestLinked ? (
          renderReadonlyInvestmentCell(
            formatEur(resolveInvestmentProgramTotal(row)),
            null,
          )
        ) : (
          <input
            ...
            onChange={(event) =>
              handleInvestmentChange(rowKey, event.target.value)
            }
        ...
        {isVesinvestLinked ? (
          renderReadonlyInvestmentCell(
            displayValidationProjectNote(t, row.note) || '-',
            null,
          )
        ) : (
          <input
  ...
  return rows.map((row) => {
    ...
    return (
      <div key={rowKey} className="v2-investment-row">
        <strong className="v2-investment-year-pill">{row.year}</strong>
        {isVesinvestLinked ? (
          renderReadonlyInvestmentCell(formatEur(row.amount), null)
        ) : (
          <input
            className="v2-input"
            type="number"
        ...
        {isVesinvestLinked ? (
          ...
          <select
            ...
            aria-label={`${t('v2Forecast.investmentTypePlaceholder', 'Type')} ${
              row.year
            }`}
            value={row.investmentType ?? ''}
            onChange={(event) =>
        ...
        {isVesinvestLinked ? (
          ...
          <input
            ...
            aria-label={`${t('v2Forecast.investmentNotePlaceholder', 'Note')} ${
              row.year
            }`}
