...
1. Execute `DO` for `S-03` to restore deterministic root lint green state.
2. Execute `S-04` to restore deterministic root typecheck green state.
3. Execute `S-05` to enforce deterministic `release-check` gate ordering and full PASS evidence.
4. Re-validate root gates (`pnpm lint`, `pnpm typecheck`, `pnpm release-check`) after S-03/S-04.
5. Customer TBD items B-TBD-01..B-TBD-05 for final acceptance lock.
