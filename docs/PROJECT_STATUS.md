...
- `S-37` substep 4 is committed and reviewed, but the row is blocked in review: shell regressions pass, yet the old import panels still expose multiple primary CTAs, so row acceptance is not satisfied and no unchecked `S-37` substep remains.
...
4. `S-37` is blocked by a sprint-scope gap: the shell work landed, but the current row still needs explicit CTA cleanup across the import panels and has no unchecked substep left to execute.

## Next actions

1. Run a focused `PLAN` pass to extend `S-37` with an executable CTA-cleanup substep or split that work into the next row before resuming `RUNSPRINT`.
