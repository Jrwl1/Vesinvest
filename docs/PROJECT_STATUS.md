...
- `S-35` is in progress: component-layer fallback copy now routes through existing locale keys, translated source/domain labels normalize backend dataset tokens plus provenance/risk labels, and the next step is the full FI/SV/EN locale sweep.
...
2. No active protocol blocker is open; `S-35` can continue immediately from the locale-sweep substep.
...
1. Keep the tracked working tree clean, then continue `DO` or `RUNSPRINT` from `S-35` substep 3.
2. Preserve the accepted Forecast trust model while completing the FI/SV/EN locale sweep across login, Overview, Forecast, and Reports.
