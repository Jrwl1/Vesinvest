...
| S-27 | Refresh Forecast with the same visual system after Overview is accepted, keeping the screen editor-first and scenario-driven. | apps/web/src/v2/EnnustePageV2.tsx, apps/web/src/v2/v2.css, apps/web/src/v2/AppShellV2.tsx, apps/web/src/i18n/locales/, apps/web/src/api.ts | Forecast uses the refreshed layout while preserving scenario editing, assumptions, investments, fee sufficiency, risk comparison, compute/report readiness, and current projection semantics. | Not eligible (status != READY). Forecast now has the refreshed sidebar/editor shell in `0ec57ec`, the clearer scenario navigation/header surface in `784a9f0`, and the top-of-screen provenance/KPI summary in `cfb7cfc`; continue input-versus-output workspace separation work. | Stop if the visual refresh requires changing projection math, scenario payload contracts, or report freshness rules. | IN_PROGRESS |
