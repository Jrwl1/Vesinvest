...
- [x] Implement repository upsert strategy per imported year and budget naming rule for chosen org
...
- [x] Remove confirm-path persistence of revenue drivers from KVA flow
  ...
  - evidence: commit:e7affe1 | run:pnpm --filter ./apps/api test -> PASS 47 tests | files:budgets.repository.ts budgets.repository.spec.ts | docs:N/A | status: clean
- [x] Keep legacy account-line import endpoint separate while removing KVA confirm accountLines branch
  ...
  - evidence: commit:ca7d754 | run:pnpm --filter ./apps/api test -> PASS 47 tests | files:budgets.repository.ts budgets.repository.spec.ts | docs:N/A | status: clean
- [x] Run confirm-mapping regression bundle for create and update behavior across 3 years
  ...
  - evidence: commit:6dd4909 | run:pnpm --filter ./apps/api test -> PASS 54 tests | files:budgets.repository.spec.ts | docs:N/A | status: clean
| `apps/api/src/budgets/budgets.controller.ts`, `apps/api/src/budgets/budgets.service.ts`, `apps/api/src/budgets/budgets.repository.ts`, `apps/web/src/api.ts`, `apps/api/src/budgets/*.spec.ts` | Confirm writes imported totals per historical year into Talousarvio deterministically (create/update), with hierarchy preserved and no KVA-import Tuloajuri/account-line writes. | Substep 1-7 done (ea2777f..6dd4909). Acceptance: upsert per year, hierarchy preserved, no Tuloajuri/account-line writes. | Stop if persistence semantics require schema migration outside sprint scope; record scope gap and stop. | DONE |
| S-03 | Redesign KVA import modal for year-by-year totals preview and simplified apply flow.
- [x] Remove Tuloajurit table and related editable driver state from KVA modal
...
- [x] Render extracted totals as 3 historical year sections/cards with clear hierarchy display
  - files: apps/web/src/components/KvaImportPreview.tsx, apps/web/src/App.css
  - run: pnpm --filter ./apps/web typecheck
  - evidence: commit:78da80c | run:pnpm --filter ./apps/web typecheck -> PASS | files:KvaImportPreview.tsx App.css | docs:N/A | status: clean
- [x] Add explicit confirmation copy: "Your Excel produced these numbers per year before applying"
  ...
  - run: pnpm --filter ./apps/web typecheck
  - evidence: commit:50e1438 | run:pnpm --filter ./apps/web typecheck -> PASS | files:KvaImportPreview.tsx fi.json sv.json en.json | docs:N/A | status: clean
- [x] Align BudgetPage import entry points so KVA flow is totals-only and Tulot remains manual driver editing
