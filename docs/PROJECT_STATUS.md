...
- `S-27` is `IN_PROGRESS`: Forecast now separates planning inputs from outputs and uses a denser investment workspace, while the remaining risk/chart/locale/regression substeps stay active.
...
1. Continue execution from `S-27` substep 6 in `docs/SPRINT.md` using `DO` or explicit whole-sprint `RUNSPRINT`.
