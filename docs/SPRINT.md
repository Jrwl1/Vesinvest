...
- [x] Present the full canonical `Poistosuunnitelmat` row set as grouped org-wide defaults inside `Ennuste`
  ...
  - evidence: row:08cc3a5 | run:pnpm --filter ./apps/web test -- src/v2/EnnustePageV2.test.tsx src/i18n/locales/localeIntegrity.test.ts && pnpm --filter ./apps/web typecheck && pnpm --filter ./apps/api test -- src/v2/v2.service.spec.ts && pnpm --filter ./apps/api typecheck -> pass | files:apps/web/src/v2/EnnustePageV2.tsx, apps/web/src/v2/v2.css | docs:N/A | status: clean
