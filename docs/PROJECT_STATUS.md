...
- `S-29` is `IN_PROGRESS`: shared V2 spacing, chrome, shell navigation/drawer behavior, badge semantics, and locale copy are now aligned; responsive cleanup remains.
...
1. Continue execution from `S-29` substep 5 in `docs/SPRINT.md` using `DO` or explicit whole-sprint `RUNSPRINT`.
