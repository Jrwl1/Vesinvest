...
| S-77 | Rebuild the shared year-review surface so correction choices are explicit and full manual override is truly complete. See S-77 substeps. | apps/web/src/v2/OverviewPageV2.tsx, apps/web/src/v2/yearReview.ts, apps/web/src/v2/v2.css, apps/web/src/i18n/locales/*.json, apps/web/src/v2/OverviewPageV2.test.tsx, apps/web/src/v2/yearReview.test.ts | Year review starts with business summary and visible primary actions; full manual override exposes all finance-critical inputs regardless of completeness; provenance and restore paths remain visible but secondary. | Packet 1-3 complete; see substep evidence. | Stop if a full override surface cannot be made truthful without changing the current imported-year storage contract beyond this queue. | READY |
