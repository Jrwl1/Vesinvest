...
- [ ] Run a fresh local browser review-loop audit from `Tili -> Tyhjennä tietokanta` through steps `1..6` and record the explicit sprint outcome in `docs/WIZARD_REVIEW_LOOP_REAUDIT.md`
  ...
  - evidence: HARD BLOCKED: local browser audit could not complete because the available frontend sessions resolved `/api/*` through a different `localhost:3000` service than the healthy backend on `127.0.0.1:3000`, and the Playwright transport crashed during the request-interception workaround; see `docs/WIZARD_REVIEW_LOOP_REAUDIT.md`.
