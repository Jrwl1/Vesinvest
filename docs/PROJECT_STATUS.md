...
1. No sprint row is currently `READY`; remaining active row is `S-05` (`TODO`).
2. `S-05` still has `Evidence needed` for acceptance review.
3. Customer-owned TBD items `B-TBD-01..B-TBD-05` remain open for final acceptance lock.

## Next 5 actions

1. Execute `DO` for `S-05` substep 1 (define build gate command checklist in deployment runbook).
2. Continue `S-05` substeps sequentially with commit-per-substep evidence.
3. Move `S-05` to `READY` only after all substeps are checked with `commit | run | files`.
4. Run `REVIEW` on `S-05` once it becomes `READY` to verify acceptance for `DONE`.
