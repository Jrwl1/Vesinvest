...
import {
  ...
  type YearEditorState,
} from '../components/EnnusteYearEditorDrawer';
...
const AUTO_BOOTSTRAP_FLAG = String(
  import.meta.env.VITE_PROJECTION_AUTO_BOOTSTRAP ?? 'true',
).toLowerCase();
const AUTO_BOOTSTRAP_ENABLED = !['0', 'false', 'off'].includes(
  AUTO_BOOTSTRAP_FLAG,
);

type ScenarioDriverDraft = Record<
  'vesi' | 'jatevesi',
  { yksikkohinta: string; myytyMaara: string }
>;
...
function yearOverridesToInvestments(
  ...
  return Object.entries(overrides)
    .map(([yearKey, value]) => ({
      year: Number(yearKey),
      amount:
        typeof value?.investmentEur === 'number' ? value.investmentEur : NaN,
    }))
    .filter(
      (item) => Number.isFinite(item.year) && Number.isFinite(item.amount),
    );
}
...
export const ProjectionPage: React.FC = () => {
  ...
  const [activeProjection, setActiveProjection] = useState<Projection | null>(
    null,
  );
  ...
  const [driverPaths, setDriverPaths] = useState<DriverPaths | undefined>(
    undefined,
  );
  ...
  const [newScenarioDrivers, setNewScenarioDrivers] =
    useState<ScenarioDriverDraft>(EMPTY_SCENARIO_DRIVER_DRAFT);
  const [newScenarioInvestments, setNewScenarioInvestments] = useState<
    Array<{ year: number; amount: number }>
  >([]);
  ...
  const plannerYears = useMemo(() => {
    if (activeProjection?.talousarvio?.vuosi != null) {
      ...
      return Array.from(
        { length: horizon + 1 },
        (_, idx) => activeProjection.talousarvio!.vuosi + idx,
      );
    }
    return (activeProjection?.vuodet ?? []).map((y) => y.vuosi);
  }, [
    activeProjection?.talousarvio?.vuosi,
    activeProjection?.aikajaksoVuosia,
    activeProjection?.vuodet,
  ]);

  const driverPathsDirty = useMemo(() => {
    return (
      stableStringifyPaths(driverPaths) !==
      stableStringifyPaths(activeProjection?.ajuriPolut)
    );
  }, [driverPaths, activeProjection?.ajuriPolut]);

  const formatRequiredDriverLabel = useCallback(
    (path: string) => {
  ...
  const captureComputeValidation = useCallback(
    (rawError: any) => {
      ...
      const message =
        typeof details?.remediation === 'string'
          ...
          ? details.message
          : rawError?.message || t('projection.errorComputeFailed');
      const requiredMissing = Array.isArray(details?.requiredMissing)
        ? details.requiredMissing.filter(
  ...
  const applyProjectionSelection = useCallback((full: Projection) => {
    ...
    const existingOverrides =
      (full.olettamusYlikirjoitukset as Record<string, number>) ?? {};
    ...
    for (const key of ASSUMPTION_KEYS) {
      overrideState[key] =
        key in existingOverrides ? existingOverrides[key] : null;
    }
  ...
  const selectOrBootstrapProjection = useCallback(
    async (projList: Projection[], budgetList: Budget[]) => {
      if (projList.length > 0) {
        const defaultProjection =
          projList.find((p) => p.onOletus) ??
          [...projList].sort(
      ...
      setBootstrappingProjection(true);
      try {
        const bootstrappedProjection = await computeForBudget(
          baselineBudget.id,
        );
        ...
        if (refreshed.length > 0) {
          const fallback =
            refreshed.find((projection) => projection.onOletus) ??
            [...refreshed].sort(
              (a, b) => Date.parse(b.createdAt) - Date.parse(a.createdAt),
            )[0];
  ...
  useEffect(() => {
    if (
      loading ||
      bootstrappingProjection ||
      activeProjection ||
      projections.length === 0
    )
      return;
    const fallback =
      projections.find((projection) => projection.onOletus) ??
      [...projections].sort(
        (a, b) => Date.parse(b.createdAt) - Date.parse(a.createdAt),
      )[0];
  ...
  useEffect(() => {
    if (
      ...
      (effectiveSelectedYear == null ||
        !years.some((y) => y.vuosi === effectiveSelectedYear))
    ) {
  ...
  const createModalBaseYear =
    selectedCreateBudget?.vuosi ??
    activeProjection?.talousarvio?.vuosi ??
    new Date().getFullYear();
  const createModalYears = useMemo(
    () =>
      Array.from(
        { length: Math.max(1, newHorizon) + 1 },
        (_, index) => createModalBaseYear + index,
      ),
  ...
  const openCreateScenarioForm = useCallback(() => {
    ...
    const readPathValue = (
      ...
      if (
        plan.mode === 'percent' &&
        typeof plan.baseValue === 'number' &&
        Number.isFinite(plan.baseValue)
      )
        return plan.baseValue;
      return undefined;
    };
    ...
    setNewScenarioDrivers({
      ...
      jatevesi: {
        yksikkohinta: formatDraftNumber(
          readPathValue('jatevesi', 'yksikkohinta'),
        ),
        myytyMaara: formatDraftNumber(readPathValue('jatevesi', 'myytyMaara')),
      },
    });
    const inheritedInvestments = Array.isArray(
      activeProjection?.userInvestments,
    )
      ? activeProjection.userInvestments.filter(
          (item) => Number.isFinite(item.year) && Number.isFinite(item.amount),
        )
      : [];
    setNewScenarioInvestments(
      inheritedInvestments.length > 0
        ? inheritedInvestments.map((item) => ({
  ...
  const handleAddScenarioInvestmentDraft = () => {
    const nextYear =
      createModalYears.find(
        (year) => !newScenarioInvestments.some((item) => item.year === year),
      ) ?? createModalBaseYear;
  ...
  const handleScenarioInvestmentDraftChange = (
    index: number,
    field: 'year' | 'amount',
    value: number,
  ) => {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      const firstAvailableScenarioName = buildUniqueScenarioName(
        baseName,
        projections,
      );
      const scenarioDriverPaths = buildScenarioDriverPaths(
        newScenarioDrivers,
        createModalBaseYear,
      );
      const scenarioInvestments = newScenarioInvestments
        .map((item) => ({
          year: Math.round(Number(item.year)),
          amount: Number(item.amount),
        }))
        .filter(
          (item) =>
      ...
      const createAndComputeScenario = async (scenarioName: string) => {
        const scenarioPayload = {
          ...
          userInvestments:
            scenarioInvestments.length > 0 ? scenarioInvestments : undefined,
        };
      ...
      for (let attempt = 0; attempt < 6; attempt += 1) {
        try {
          ...
          const duplicateName =
            status === 409 ||
            /already exists|duplicate|unique|conflict/i.test(msg);
  ...
  const ensureEditableProjection =
    useCallback(async (): Promise<Projection | null> => {
      ...
      const existingWorking = projections
        .filter(
          (projection) =>
            !projection.onOletus &&
            projection.talousarvioId === activeProjection.talousarvioId,
        )
      ...
      const baseYear =
        activeProjection.talousarvio?.vuosi ?? new Date().getFullYear();
      const workingName = buildUniqueScenarioName(
        `Tyoversio ${baseYear}`,
        projections,
      );
      const created = await createProjection({
        ...
        olettamusYlikirjoitukset:
          activeProjection.olettamusYlikirjoitukset ?? undefined,
        ajuriPolut: activeProjection.ajuriPolut ?? undefined,
        userInvestments: Array.isArray(activeProjection.userInvestments)
          ? activeProjection.userInvestments
          : undefined,
        vuosiYlikirjoitukset:
          activeProjection.vuosiYlikirjoitukset ?? undefined,
      });
  ...
  const handleDelete = async () => {
    ...
    if (
      !window.confirm(
        t('projection.deleteConfirm', { name: activeProjection.nimi }),
      )
    )
      return;
    try {
      ...
      if (projList.length > 0) {
        const fallback =
          projList.find((projection) => projection.onOletus) ??
          [...projList].sort(
            (a, b) => Date.parse(b.createdAt) - Date.parse(a.createdAt),
          )[0];
  ...
  const handleCompute = async () => {
    ...
    try {
      if (
        activeProjection.onOletus &&
        (hasOverrides || hasYearOverrides || hasUserInvestments)
      ) {
  ...
  const handleAddUserInvestment = () => {
    ...
    const nextYear =
      years.find((y) => !userInvestments.some((u) => u.year === y)) ?? baseYear;
  ...
  const handleUserInvestmentChange = (
    index: number,
    field: 'year' | 'amount',
    value: number,
  ) => {
  ...
  const setYearOverride = useCallback(
    (
      year: number,
      updater: (prev: ProjectionYearOverride) => ProjectionYearOverride,
    ) => {
      setYearOverrides((prev) => {
        const current = prev[year] ?? {};
        const nextValue = updater(current);
        const hasCategory = Boolean(
          nextValue.categoryGrowthPct &&
            Object.keys(nextValue.categoryGrowthPct).length > 0,
        );
        const hasLines = Boolean(
          nextValue.lineOverrides &&
            Object.keys(nextValue.lineOverrides).length > 0,
        );
        const hasCore =
  ...
  const handleSaveUserInvestments = async () => {
    ...
    try {
      ...
      if (!editableProjection) return;
      const updated = await updateProjection(editableProjection.id, {
        userInvestments: userInvestments.filter(
          (u) => u.amount !== 0 || u.year > 0,
        ),
      });
      setActiveProjection(updated);
            ...
            <thead>
              <tr>
                ...
                <th className="num-col">
                  {t('projection.columns.investments')}
                </th>
                <th className="num-col result-col">
                  {t('projection.columns.netResult')}
                </th>
                <th className="num-col">
                  {t('projection.columns.cumulative')}
                </th>
                <th className="num-col">
                  {t('projection.columns.waterPrice')}
                </th>
  ...
  const selectedYearData =
    effectiveSelectedYear != null
      ...
      : null;
  const selectedYearOverride =
    effectiveSelectedYear != null
      ? yearOverrides[effectiveSelectedYear]
      : undefined;
  ...
  const computedGrowthPct =
    selectedYearData && previousYearData && num(previousYearData.vesihinta) > 0
      ? (num(selectedYearData.vesihinta) / num(previousYearData.vesihinta) -
          1) *
        100
      : null;
  const selectedYearLineKeys = selectedYearData?.erittelyt
    ? Array.from(
        new Set([
          ...
          ...(selectedYearData.erittelyt.investoinnit?.map(
            (line) => line.nimi,
          ) ?? []),
        ]),
      ).filter(Boolean)
    : [];
  const yearEditorState: YearEditorState | null =
    effectiveSelectedYear != null && selectedYearData
      ? {
  ...
  const tariffikorotus =
    baseYearTariff > 0 && requiredTariff > 0
  ...
  const selectedYearCashflow = selectedYearData
    ...
    : null;
  const selectedYearInvestments = selectedYearData
    ? num(selectedYearData.investoinnitYhteensa)
    : null;
  const selectedYearWaterUnitPrice =
    selectedYearData?.erittelyt?.ajurit?.find(
      (driver) => driver.palvelutyyppi === 'vesi',
    )?.yksikkohinta ?? null;
  const selectedYearWastewaterUnitPrice =
    selectedYearData?.erittelyt?.ajurit?.find(
      (driver) => driver.palvelutyyppi === 'jatevesi',
    )?.yksikkohinta ?? null;
  const selectedYearWaterVolume =
    selectedYearData?.erittelyt?.ajurit?.find(
      (driver) => driver.palvelutyyppi === 'vesi',
    )?.myytyMaara ?? null;
  const selectedYearWastewaterVolume =
    selectedYearData?.erittelyt?.ajurit?.find(
      (driver) => driver.palvelutyyppi === 'jatevesi',
    )?.myytyMaara ?? null;
  const selectedYearCombinedWeighted =
    ...
    selectedYearWaterVolume + selectedYearWastewaterVolume > 0
      ...
      : selectedYearData
      ? num(selectedYearData.vesihinta)
      : null;
  const selectedYearCombinedFormula =
    ...
    selectedYearWastewaterVolume != null &&
    selectedYearWaterVolume + selectedYearWastewaterVolume > 0
      ? `(${formatTariffEurPerM3(selectedYearWaterUnitPrice)} * ${formatM3Int(
          selectedYearWaterVolume,
        )} + ${formatTariffEurPerM3(
          selectedYearWastewaterUnitPrice,
        )} * ${formatM3Int(selectedYearWastewaterVolume)}) / ${formatM3Int(
          selectedYearWaterVolume + selectedYearWastewaterVolume,
        )}`
      : undefined;
  const baseProjectionYear =
    activeProjection?.talousarvio?.vuosi ?? years[0]?.vuosi ?? null;
  // --- V2 render ---
  ...
  return (
    <div className="projection-page ev2-page">
        ...
        <div className="ev2-topbar__left">
          ...
          {activeProjection && (
            <span className="ev2-topbar__meta">
              {tv2('budgetLabel')}:{' '}
              <strong>
                {activeProjection.talousarvio?.nimi ??
                  activeProjection.talousarvioId}
              </strong>
              {' · '}
              {tv2('horizonLabel')}:{' '}
              <strong>
                {activeProjection.aikajaksoVuosia} {tv2('horizonUnit')}
              </strong>
            </span>
          )}
        </div>
          ...
          {hasComputedData && (
            <>
              ...
              <button
                type="button"
                className="ev2-btn ev2-btn--ghost"
                data-testid="projection-export-pdf-btn"
      ...
      {error && (
        <div className="ev2-error-banner" role="alert">
          {error}
        </div>
      )}
      ...
      <div className="ev2-scenario-row">
        <span className="ev2-scenario-row__label">
          {tv2('scenariosLabel')}:
        </span>
        <EnnusteScenarioRow
          ...
          onDeleteScenario={
            activeProjection && !activeProjection.onOletus
              ? handleDelete
              : undefined
          }
          ...
          deleteScenarioAriaLabel={
            activeProjection
              ? t('projection.deleteScenarioAria', {
                  name: activeProjection.nimi,
                })
              : undefined
          }
        />
      ...
      {showCreateForm && (
        <div
          ...
          onClick={(e) =>
            e.target === e.currentTarget && resetCreateScenarioForm()
          }
        >
          <div className="modal-content create-scenario-form card">
            <h3 id="create-scenario-modal-title">
              {t('projection.createScenario')}
            </h3>
            <p className="create-scenario-form__context">
              {t('projection.createScenarioContext')}
            </p>
            ...
            <div className="form-row">
              ...
              <div className="horizon-input">
                <input
                  ...
                  value={newHorizon}
                  onChange={(e) =>
                    setNewHorizon(parseInt(e.target.value) || 20)
                  }
                />
                ...
                {(['vesi', 'jatevesi'] as const).map((service) => (
                  <div key={service} className="create-scenario-driver-card">
                    <strong>
                      {t(
                        service === 'vesi'
                          ? 'projection.driverPlanner.water'
                          : 'projection.driverPlanner.wastewater',
                      )}
                    </strong>
                    <label>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          handleScenarioDriverDraftChange(
                            service,
                            'yksikkohinta',
                            e.target.value,
                          )
              ...
              <table className="financing-investments-table financing-investments-table--compact">
                <thead>
                  <tr>
                    ...
                    <th className="num-col">
                      {t('projection.financing.amount')} (EUR)
                    </th>
                    <th></th>
                  </tr>
                </thead>
                    ...
                    <tr key={`${item.year}-${index}`}>
                      <td>
                        <select
                          value={item.year}
                          onChange={(e) =>
                            handleScenarioInvestmentDraftChange(
                              index,
                              'year',
                      ...
                      <td>
                        <button
                          ...
                          className="btn-link"
                          onClick={() =>
                            handleRemoveScenarioInvestmentDraft(index)
                          }
                        >
                          {t('common.delete')}
                        </button>
            ...
            <div className="form-actions">
              <button
                ...
                onClick={resetCreateScenarioForm}
              >
                {t('common.cancel')}
              </button>
              <button
        ...
        <EnnusteTuloksetZone heading={tv2('resultsZoneTitle')}>
          {hasComputedData && (
            <div className="ev2-kpi-strip" role="status" aria-live="polite">
              <div className="ev2-kpi ev2-kpi--primary">
                <span className="ev2-kpi__label">
                  {tv2('kpiRequiredTariff')}
                </span>
                <span className="ev2-kpi__value">
                  {formatTariffEurPerM3(
                    activeProjection.requiredTariff ?? undefined,
                  )}
                </span>
              </div>
              <div
                className="ev2-kpi"
                title={
                  selectedYearCombinedFormula
                    ? `${tv2(
                        'kpiCombinedWeightedHelp',
                      )}: ${selectedYearCombinedFormula}`
                    : undefined
                }
              >
                <span className="ev2-kpi__label">
                  {tv2('kpiCombinedWeighted')}
                </span>
                <span className="ev2-kpi__value">
                  {formatTariffEurPerM3(
                    selectedYearCombinedWeighted ?? undefined,
                  )}
                </span>
              </div>
              <div className="ev2-kpi">
                <span className="ev2-kpi__label">
                  {tv2('kpiSelectedWaterPrice')}
                </span>
                <span className="ev2-kpi__value">
                  {formatTariffEurPerM3(
                    selectedYearWaterUnitPrice ?? undefined,
                  )}
                </span>
              </div>
              <div className="ev2-kpi">
                <span className="ev2-kpi__label">
                  {tv2('kpiSelectedWastewaterPrice')}
                </span>
                <span className="ev2-kpi__value">
                  {formatTariffEurPerM3(
                    selectedYearWastewaterUnitPrice ?? undefined,
                  )}
                </span>
              </div>
              {tariffikorotus !== null && (
                <div className="ev2-kpi">
                  <span className="ev2-kpi__label">
                    {tv2('kpiTariffikorotus')}
                  </span>
                  <span
                    className={`ev2-kpi__value ${
                      tariffikorotus > 0 ? 'ev2-negative' : 'ev2-positive'
                    }`}
                  >
                    {tariffikorotus > 0 ? '+' : ''}
                    {tariffikorotus.toFixed(1)} %
                  </span>
                </div>
              )}
              <div className="ev2-kpi">
                ...
                <span className="ev2-kpi__value">
                  {formatTariffEurPerM3(tariffYearPlusOne)}
                </span>
              </div>
              <div className="ev2-kpi">
                ...
                <span
                  className={`ev2-kpi__value ${
                    finalCumulative >= 0 ? 'ev2-positive' : 'ev2-negative'
                  }`}
                >
              ...
              <div className="ev2-kpi">
                ...
                <span className="ev2-kpi__value">
                  {formatEurInt(selectedYearInvestments)}
                </span>
              </div>
              <div className="ev2-kpi">
              ...
              <label className="ev2-kpi ev2-kpi--select">
                ...
                >
                  {years.map((y) => (
                    <option key={y.vuosi} value={y.vuosi}>
                      {y.vuosi}
                    </option>
                  ))}
                </select>
          ...
          <EnnusteYearEditorDrawer
            ...
            onSetWaterPrice={(value) => {
              if (effectiveSelectedYear == null) return;
              const prevPrice = previousYearData
                ? num(previousYearData.vesihinta)
                : 0;
              const growth =
                prevPrice > 0 ? (value / prevPrice - 1) * 100 : undefined;
            ...
            onSetGrowthPct={(value) => {
              ...
              const prevPrice = previousYearData
                ...
                : 0;
              const price =
                prevPrice > 0 ? prevPrice * (1 + value / 100) : undefined;
              setYearOverride(effectiveSelectedYear, (prev) => ({
                ...prev,
      ...
      {activeProjection && (
        <EnnusteSyotaZone heading={tv2('inputsTitle')}>
          <p className="ev2-input-hint">
            {tv2('onboardingHint')} {tv2('baselineYearLabel')}:{' '}
            {baseProjectionYear ?? '-'}
            {' · '}
            {tv2('horizonLabel')}: {activeProjection.aikajaksoVuosia}{' '}
            {tv2('horizonUnit')}
          </p>
          <div className="ev2-body-grid">
              ...
              {hasComputedData ? (
                <div className="ev2-year-cards__list">
                  {years.map((y, idx) => {
                    ...
                    const kv =
                      typeof y.kassafloede === 'number'
                        ? y.kassafloede
                        : tulos - num(y.investoinnitYhteensa);
                    ...
                    return (
                      <button
                        key={y.vuosi}
                        type="button"
                        className={`ev2-year-card${
                          isSelected ? ' ev2-year-card--selected' : ''
                        }${tulos < 0 ? ' ev2-year-card--deficit' : ''}`}
                      ...
                      >
                        <div className="ev2-year-card__header">
                          ...
                          {isBase && (
                            <span className="ev2-badge ev2-badge--base">
                              perusta
                            </span>
                          )}
                          {tulos < 0 && !isBase && (
                            <span className="ev2-badge ev2-badge--deficit">
                              alijäämä
                            </span>
                          )}
                        </div>
                        {num(y.vesihinta) > 0 && (
                          <div className="ev2-year-card__row ev2-year-card__row--tariff">
                            ...
                            <span>
                              {formatTariffEurPerM3(num(y.vesihinta))}
                            </span>
                          </div>
                        )}
                        ...
                        <div
                          className={`ev2-year-card__row ev2-year-card__row--kv ${
                            kv >= 0 ? 'ev2-positive' : 'ev2-negative'
                          }`}
                        >
            ...
            <div className="ev2-inputs">
              ...
              <div className="ev2-compute-bar">
                ...
                {hasComputedData && activeProjection.updatedAt && (
                  <span
                    className="ev2-last-computed"
                    role="status"
                    data-testid="projection-last-computed"
                  >
                    {t('projection.lastComputed')}:{' '}
                    {new Date(activeProjection.updatedAt).toLocaleString(
                      'fi-FI',
                ...
                {driverPathsDirty && (
                  <span className="ev2-dirty-hint" role="alert">
                    {tv2('saveBeforeCompute')}
                  </span>
                )}
                {horizonChangedNotice && !driverPathsDirty && (
                  <span className="ev2-dirty-hint" role="status">
              ...
              {computeValidation && (
                <div
                  className="ev2-validation-banner"
                  role="alert"
                  data-testid="projection-validation-banner"
                >
                  <div className="ev2-validation-banner__content">
                    <strong>
                      {t(
                        'projection.v2.validationFixTitle',
                        'Laskenta estettiin ennen virheellistä ennustetta',
                      )}
                    </strong>
                    <span>{computeValidation.message}</span>
                    {computeValidation.requiredMissing &&
                      computeValidation.requiredMissing.length > 0 && (
                        <span>
                          {computeValidation.requiredMissing
                            .slice(0, 4)
                            .map(formatRequiredDriverLabel)
                            .join(' · ')}
                        </span>
                      )}
                  </div>
                  ...
                  >
                    {t(
                      'projection.v2.openBudgetFixCta',
                      'Avaa Talousarvio ja korjaa',
                    )}
                  </button>
                </div>
              )}
              ...
              <section className="ev2-input-section">
                <h2 className="ev2-input-section__title">
                  {tv2('assumptionsTitle')}
                </h2>
                <div className="ev2-assumptions-grid">
                  {(
                    [
                      {
                        key: 'vesimaaran_muutos',
                        label: tv2('assumptionVesimaara'),
                      },
                      { key: 'inflaatio', label: tv2('assumptionKayttomenot') },
                      {
                        key: 'energiakerroin',
                        label: tv2('assumptionEnergia'),
                      },
                      {
                        key: 'henkilostokerroin',
                        label: tv2('assumptionHenkilosto'),
                      },
                      { key: 'hintakorotus', label: tv2('assumptionTariffi') },
                      {
                        key: 'investointikerroin',
                        label: tv2('assumptionInvestointi'),
                      },
                    ] as const
                  ).map(({ key, label }) => {
                    ...
                    return (
                      <div
                        key={key}
                        className={`ev2-assumption-row${
                          hasOv ? ' ev2-assumption-row--overridden' : ''
                        }`}
                      >
                ...
                <div className="ev2-horizon-row">
                  <label
                    htmlFor="ev2-horizon-select"
                    className="ev2-assumption-label"
                  >
                    {tv2('horizonLabel')}
                  </label>
                  <select
                    ...
                    onChange={(e) =>
                      handleHorizonChange(parseInt(e.target.value, 10))
                    }
                  >
              ...
              {hasComputedData && (
                <section className="ev2-input-section">
                  <details className="revenue-report-section">
                    <summary className="revenue-report-toggle">
            ...
            {hasComputedData && (
              <div className="ev2-extra-col">
                <div
                  className="ev2-input-section__title"
                ...
                >
                  {years.map((y) => {
                    ...
                    const kv =
                      typeof y.kassafloede === 'number'
                        ? y.kassafloede
                        : num(y.tulos) - num(y.investoinnitYhteensa);
                    const isSelected = y.vuosi === effectiveSelectedYear;
                    return (
                      <button
                        key={y.vuosi}
                        type="button"
                        onClick={() => setSelectedYear(y.vuosi)}
                        className={`ev2-timeline-row${
                          isSelected ? ' ev2-timeline-row--selected' : ''
                        }${kv < 0 ? ' ev2-timeline-row--deficit' : ''}`}
                      >
                        <span className="ev2-timeline-row__year">
                          {y.vuosi}
                        </span>
                        <span className="ev2-timeline-row__tariff">
                          {tariffi > 0 ? formatTariffEurPerM3(tariffi) : '-'}
                        </span>
                        <span
                          className={`ev2-timeline-row__kv ${
                            kv >= 0 ? 'ev2-positive' : 'ev2-negative'
                          }`}
                        >
                          {formatEurInt(kv)}
        ...
        <div className="ev2-empty-state">
          <p>
            {AUTO_BOOTSTRAP_ENABLED
              ? t('projection.bootstrapPendingHint')
              : t('projection.noDataHint')}
          </p>
...
function buildScenarioDriverPaths(
  draft: ScenarioDriverDraft,
  baseYear: number,
): DriverPaths | undefined {
  ...
  for (const service of ['vesi', 'jatevesi'] as const) {
    ...
    if (price !== undefined) {
      servicePlan.yksikkohinta = {
        mode: 'manual',
        values: { [baseYear]: price },
      };
    }
    if (volume !== undefined) {
      servicePlan.myytyMaara = {
        mode: 'manual',
        values: { [baseYear]: volume },
      };
    }
