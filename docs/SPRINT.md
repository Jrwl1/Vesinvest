...
- [ ] Add report freshness token validation and canonical report total derivation from snapshot investments
  ...
  - evidence: commit:<hash> | run:pnpm --filter ./apps/api typecheck && pnpm --filter ./apps/web typecheck -> <result> | files:<actual changed paths> | docs:<hash or N/A> | status: clean

### S-12 substeps

- [ ] Rework Forecast compute/report gating so report creation only uses explicit compute state
  ...
  - evidence: commit:<hash> | run:pnpm --filter ./apps/web typecheck && pnpm --filter ./apps/web test -- src/v2/AppShellV2.test.tsx -> <result> | files:<actual changed paths> | docs:<hash or N/A> | status: clean

### S-13 substeps

- [ ] Clear stale scenario editor content during scenario switch and lock editing controls until fresh payload is loaded
  - files: apps/web/src/v2/EnnustePageV2.tsx, apps/web/src/v2/v2.css
  - run: pnpm --filter ./apps/web typecheck
  - evidence: commit:<hash> | run:pnpm --filter ./apps/web typecheck -> <result> | files:<actual changed paths> | docs:<hash or N/A> | status: clean

### S-14 substeps

- [ ] Add investment bulk actions and safe numeric guardrails in yearly investments editor
