...
| S-150 | Fix trusted client IP derivation and move auth throttling to a production-safe shared or edge-backed contract. See S-150 substeps. | apps/api/src/auth/**, apps/api/src/main.ts, apps/api/package.json, apps/api/.env.example, apps/api/src/**/*.spec.ts, DEPLOYMENT.md, README.md | Login, demo-login, and invitation acceptance no longer key off spoofable forwarded headers, and production auth throttling is no longer only per-process memory without an explicit shared or edge-backed enforcement path. | Packet 1 complete; shared-throttle contract packet pending. | Stop if the current production target cannot provide any safe shared limiter backend or verified edge-enforcement path; record the exact deployment dependency before stopping. | IN_PROGRESS |
