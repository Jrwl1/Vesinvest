...
|------|-------------|------------------|--------|
| **1** | **Row-76-style header:** In KVA adapter, add detection: row contains "Budget" (BUDGET_HEADER) and next row has numeric account in col0 → treat as section header; colMap tiliryhma=0, nimi=1, summa=index of Budget cell. Parse Blad1 rows 77–87; stop at 88 (total). | `apps/api/src/budgets/va-import/kva-template.adapter.ts` | Unit test: sheet with row 76 = [Förverkligat, Vatten, 2023, Budget], row 77 = [6800, Maskinhyror, 266, 5000]; expect 1 line, summa=5000, no "Could not detect Budget" warning. Fixture test (skip if missing): load KVA.xlsx, expect ≥1 line from Blad1, amountColumnUsed includes "Budget". |
| 2 | Multiple budget blocks in same sheet: keep scanning all section headers (existing + row-76-style); parse each block; aggregate lines; stop at total/blank/next header. **Implemented:** Budget lines are imported from **Blad1 only**; all blocks in that sheet are scanned and aggregated; other sheets (KVA totalt, Vatten KVA, Avlopp, etc.) are reserved for later steps (drivers/assumptions) and are not parsed for budget lines. | Same adapter. | Unit test: two blocks, two header rows. |
| 3 | Revenue drivers from KVA totalt: parse rows 55–58 for Vatten/Avlopp prices and VAT; set `VaImportPreview.revenueDrivers`. **Implemented:** Unit prices (€/m³) and VAT% are read from sheet **KVA totalt**: locate the "Pris €/m³" header row, then columns "moms 0 %" / "moms 24 %", then data rows **Vatten** and **Avlopp**. Prefer moms 0% for `yksikkohinta`; set `alvProsentti` to 24 when moms 24% column exists. Optional: volume (m³/a) and connection count from **Vatten KVA**, **Avlopp KVA**, **Anslutningar** via label scan. If KVA totalt is missing or the Pris table is not found, drivers return with empty prices and **one** warning (e.g. "Revenue drivers: could not locate … in template; leaving empty."). No DB write on confirm yet. | Adapter: `previewKvaRevenueDrivers()`. | Unit test: in-memory KVA totalt with Pris €/m³, moms 0%/24%, Vatten/Avlopp rows. Fixture test: revenueDrivers length ≥ 1, at least one with price or VAT when present. |
| 4 | Revenue drivers from Vatten KVA / Avlopp KVA and Anslutningar: volume/revenue and liittymamaara for chosen year. | Same. | Unit test: mock sheets. |
| 5 | Preview API: expose `revenueDriversPreview` (and optional `assumptionsPreview`) in `ImportPreviewResult`. | `budget-import.service.ts`, web `api.ts` types. | — |
| 6 | Confirm API: after creating lines, if preview had drivers, upsert Tuloajuri by palvelutyyppi. | `budgets.service.ts`, `budgets.repository.ts`. | Unit test: confirm with drivers. |
| 7 | Web UI: show driver summary in preview; ensure modal layout (no double scroll, sticky footer, solid panel). | `BudgetImport.tsx`, `App.css`. | Manual. |
| 8 | Fixture-based test: load KVA.xlsx; assert budget line count > 0, Blad1 in processedSheets, no "no section header" for Blad1 when file present; skip when fixture missing. | `kva-template.adapter.spec.ts` | Use `VA_FIXTURES_DIR` or `fixtures/`; skip with clear message. |
