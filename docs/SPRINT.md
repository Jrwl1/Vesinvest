...
| S-144 | Extract manual year patch and year-detail workflow state into dedicated hooks/modules. See S-144 substeps. | apps/web/src/v2/OverviewPageV2.tsx, apps/web/src/v2/yearReview.ts, apps/web/src/api.ts, apps/web/src/v2/OverviewPageV2.test.tsx, apps/web/src/v2/yearReview.test.ts, apps/web/src/v2/*.ts | Inline edit state, dirty guards, save behavior, and statement/workbook/QDIS workflow coordination are owned by dedicated hooks/modules, and the accepted step-2/step-3 behaviors from `S-137..S-141` remain intact. | Packet 1 committed in `7873a85`; row in progress. | Stop if extraction would require changing the accepted manual patch contract or provenance behavior instead of only relocating page-owned orchestration. | IN_PROGRESS |
