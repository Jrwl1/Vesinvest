...
- `S-27` is `IN_PROGRESS`: Forecast now separates planning inputs from outputs, uses a denser investment workspace, and has clearer risk comparisons; the remaining chart/locale/regression substeps stay active.
...
1. Continue execution from `S-27` substep 7 in `docs/SPRINT.md` using `DO` or explicit whole-sprint `RUNSPRINT`.
