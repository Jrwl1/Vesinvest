...
export function registerVesinvestPlanningPanelEvidenceWorkflowSuite() {
  describe('VesinvestPlanningPanel evidence and workflow', () => {
    ...
    it('shows the bound VEETI identity before the defaults and plan sections', async () => {
      ...
      expect(
        screen.getByRole('button', { name: 'Investment plan' }),
      ).toBeTruthy();
    });
    ...
    it('hides the heavy plan sections in simplified setup mode', async () => {
      ...
      expect(
        screen.queryByRole('heading', {
          name: 'Class-owned depreciation plan',
        }),
      ).toBeNull();
    ...
    it('keeps the full plan surface available for non-admin simplified setup users', async () => {
      ...
      expect(
        screen.getAllByRole('button', { name: 'Add project' }).length,
      ).toBeGreaterThan(0);
      expect(
        screen.getByRole('heading', { name: 'Grouped horizon layout' }),
      ).toBeTruthy();
      expect(
        screen.getByRole('heading', { name: 'Editable project rows' }),
      ).toBeTruthy();
    ...
    it('saves the current draft before opening pricing', async () => {
      ...
      const projectNameInput = await screen.findByDisplayValue(
        'Main rehabilitation',
      );
      ...
      const updatePayload = updateVesinvestPlanV2.mock.calls[0]?.[1] as Record<
        ...
        unknown
      >;
      const updateProjects = updatePayload.projects as Array<
    ...
    it('keeps the saved Vesinvest revision visible when Forecast sync fails after save', async () => {
      listVesinvestPlansV2.mockResolvedValue([
        makeSummary({
          baselineStatus: 'verified',
          pricingStatus: 'blocked',
      ...
      updateVesinvestPlanV2.mockResolvedValue(savedPlan);
      syncVesinvestPlanToForecastV2.mockRejectedValue(
        Object.assign(
          new Error('Baseline not verified. internal_trace=wave5'),
          {
      ...
      await waitFor(() => {
        expect(updateVesinvestPlanV2).toHaveBeenCalled();
        expect(syncVesinvestPlanToForecastV2).toHaveBeenCalledWith(
          'plan-1',
          expect.objectContaining({ compute: true }),
        );
      });
      expect(screen.getByText(/Water Utility \/ v2/i)).toBeTruthy();
      expect(
        screen.getByText(
          'Verify VEETI, PDF, Excel, or manual evidence before fee-path output is treated as final.',
        ),
      ).toBeTruthy();
      expect(screen.queryByText(/internal_trace/i)).toBeNull();
      expect(screen.queryByText('Failed to save Vesinvest plan.')).toBeNull();
    });
    ...
    it('keeps add project as the only primary CTA for an empty draft', async () => {
      ...
      const workflowRow = createPlanButton.closest(
        '.v2-actions-row',
      ) as HTMLElement;
    ...
    it('binds the VEETI utility from the in-panel lookup before plan creation', async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByRole('button', { name: /Water Utility/i }),
        ).toBeTruthy();
      });
    ...
    it('keeps fee-path blocked until there are real investment allocations', async () => {
      ...
      getVesinvestPlanV2.mockResolvedValue(
        makePlan({
          ...
          yearlyTotals: [
            { year: 2026, totalAmount: 0, waterAmount: 0, wastewaterAmount: 0 },
          ],
      ...
      render(
        <VesinvestPlanningPanel
          t={t as any}
          planningContext={
            { canCreateScenario: true, baselineYears: [] } as any
          }
          linkedOrg={linkedOrg}
          onGoToForecast={() => undefined}
          onGoToReports={() => undefined}
    ...
    it('shows accepted baseline evidence and the saved fee-path summary for a synced plan', async () => {
      ...
      const feePathTitle = await screen.findByText(
        'Saved tariff-plan recommendation',
      );
      expect(
        screen.queryByText('Linked accepted budget budget-2024'),
      ).toBeNull();
      expect(
        screen.getByText('Statement import (bokslut-2024.pdf)'),
      ).toBeTruthy();
      expect(
        screen.getAllByText('Accepted baseline years').length,
      ).toBeGreaterThan(0);
      expect(
        screen.queryByText(
      ...
      expect(
        within(feePathTitle.closest('section')!).getByText('Verified'),
      ).toBeTruthy();
      expect(
        screen.getByText(
      ...
      expect(
        screen.queryByText(
          'Sync the plan to open tariff-plan and financing results.',
        ),
      ).toBeNull();
    });
    ...
    it('shows document page and source-line provenance in the evidence section', async () => {
      ...
      expect(
        await screen.findByText(
          /(Evidence file|Lähdeaineisto|Underlag) \(baseline-2024\.pdf\).+pp\. 3, 4/,
        ),
      ).toBeTruthy();
      expect(await screen.findByText(/Revenue 95 000 EUR/)).toBeTruthy();
      expect(
        await screen.findByText(/Operating costs 70 000 EUR/),
      ).toBeTruthy();
    });
    ...
    it('keeps the statement filename visible when Excel repair is mixed into the same dataset', async () => {
      ...
      expect(
        await screen.findByText(
          /Statement PDF \+ Excel repair.+bokslut-2024\.pdf/,
        ),
      ).toBeTruthy();
      expect(
        await screen.findByText(
    ...
    it('renders the grouped 20-year layout with class subtotal rows before project rows', async () => {
          ...
          projects: [
            ...
            },
            {
              ...
              depreciationClassKey:
                wastewaterTreatmentGroup.defaultDepreciationClassKey,
      ...
      expect(
        within(matrix).getByText('Water network rehabilitation'),
      ).toBeTruthy();
    ...
    it('derives total allocation from the water and wastewater split inputs', async () => {
      ...
      fireEvent.change(
        within(allocationDialog).getByLabelText('P-001 2026 Water total'),
        {
          target: { value: '125' },
        },
      );
      fireEvent.change(
        within(allocationDialog).getByLabelText('P-001 2026 Wastewater total'),
        {
          target: { value: '75' },
        },
      );
      ...
      expect(
        (
          within(allocationDialog).getByLabelText(
            'P-001 2026 Total',
          ) as HTMLInputElement
        ).value,
      ).toBe('200');
