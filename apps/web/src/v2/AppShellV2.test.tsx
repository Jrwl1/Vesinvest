...
vi.mock('./OverviewPageV2', () => ({
  OverviewPageV2: ({ onGoToForecast }: { onGoToForecast: () => void }) => (
    <button type="button" onClick={onGoToForecast}>overview-content</button>
  ),
}));

vi.mock('./EnnustePageV2', () => ({
  EnnustePageV2: ({ onReportCreated }: { onReportCreated: (id: string) => void }) => (
    <button type="button" onClick={() => onReportCreated('report-123')}>ennuste-content</button>
  ),
}));

vi.mock('./ReportsPageV2', () => ({
  ReportsPageV2: ({ focusedReportId }: { focusedReportId: string | null }) => (
...
describe('AppShellV2', () => {
  it('renders only the 3-tab navigation', () => {
    render(
      <AppShellV2
        tokenInfo={{ sub: 'u1', org_id: 'org-1', roles: ['ADMIN'], iat: 1, exp: 9999999999 }}
        isDemoMode={false}
        onLogout={() => undefined}
      />,
    );
