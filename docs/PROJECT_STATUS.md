...
- `S-29` is `IN_PROGRESS`: shared V2 spacing, chrome, and shell navigation/drawer behavior are now aligned; badge semantics, copy tone, and responsive cleanup remain.
- `S-30` remains queued for final hardening.
...
1. Continue execution from `S-29` substep 3 in `docs/SPRINT.md` using `DO` or explicit whole-sprint `RUNSPRINT`.
