...
export function registerOverviewConnectImportSearchSelectionSuite() {
  ...
  it('does not treat available years as imported when workspaceYears is empty', async () => {
    ...
    expect(await screen.findByText(localeText('v2Overview.selectedYearsLabel'))).toBeTruthy();
    expect(screen.queryByText(localeText('v2Overview.wizardSummaryImportedYears'))).toBeNull();
    expect(
  ...
  it('renders excluded years as pois suunnitelmasta without reintroducing dashboard clutter', async () => {
    ...
    expect(
      await screen.findByText(localeText('v2Overview.reviewBucketExcludedTitle')),
    ).toBeTruthy();
    expect(
      document
