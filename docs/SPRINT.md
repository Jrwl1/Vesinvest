...
- [ ] Replace the old step-3 VEETI deviation sentence with the same short exact edited-line label used on step 2
...
- [ ] Remove leftover year-level helper sentence where exact edited lines are already shown
...
- [ ] Keep one clear back action on step 2 and step 3 instead of duplicating shell and panel back buttons
...
- [ ] Keep step-4 modal/back affordances to one truthful return path
...
- [ ] Remove duplicate QDIS action buttons from the year-decision modal so upload/import appears once
...
- [ ] Trim repeated helper text in the same modal without inventing replacement body copy
