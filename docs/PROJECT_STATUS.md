...
- `S-28` is `IN_PROGRESS`: Reports now has the refreshed two-column list/preview shell; list selection, provenance, variants, and export details remain to be reworked.
...
1. Continue execution from `S-28` substep 2 in `docs/SPRINT.md` using `DO` or explicit whole-sprint `RUNSPRINT`.
