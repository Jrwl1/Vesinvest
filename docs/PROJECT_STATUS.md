...
- `S-30` is `IN_PROGRESS`: keyboard focus, state-surface messaging, and shared-shell regression proof are accepted; cleanup and final gates remain.
...
1. Continue execution from `S-30` substep 4 in `docs/SPRINT.md` using `DO` or explicit whole-sprint `RUNSPRINT`.
