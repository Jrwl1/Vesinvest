...
| S-149 | Harden workbook and statement upload boundaries and replace or mitigate vulnerable parser paths. See S-149 substeps. | apps/api/src/v2/**, apps/api/src/budgets/va-import/**, apps/api/package.json, pnpm-lock.yaml, apps/api/src/**/*.spec.ts | Workbook and statement preview endpoints enforce strict upload bounds, reject malformed/unsupported files early, and no longer rely on known vulnerable upload/parser paths for untrusted input without a documented mitigation. | Packets 1-2 complete; READY for review. | Stop if truthful workbook preview cannot be preserved after replacing or isolating the current parser stack without a broader import-contract rewrite. | READY |
