...
4. KVA Excel import + Talousarvio baseline are locked (Option A, `KVA totalt` single-source import, historical-only Talousarvio). Ennuste page is functionally complete per `docs/PROJECTION_UX_PLAN.md`, the trust hardening queue `S-31..S-36` is accepted, the initial wizard rollout `S-37..S-42` is landed, the corrective wizard queue `S-43..S-47` is accepted, the wizard UX-coherence queue `S-48..S-52` is accepted, and the current execution target in `docs/SPRINT.md` is the power-user Forecast/resultatrakning queue (`S-53..S-58` next): unify Forecast status truth, replace the long form-first landing with a resultatrakning-first cockpit around the five key planning pillars, add drill-down editing for the four operating pillars, migrate depreciation rules from org-level to scenario-specific baseline + new-investment modeling, build the `Avskrivningar` workspace with one-investment/one-category mapping and yearly preview, and end with a fresh power-user audit artifact.
