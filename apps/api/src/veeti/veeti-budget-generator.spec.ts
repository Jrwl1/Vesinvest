...
describe('VeetiBudgetGenerator', () => {
  ...
  const generator = new VeetiBudgetGenerator(prisma, veetiService as any);
  ...
  it('maps Tilinpaatos fields to valisummat', () => {
    ...
    expect(rows).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ categoryKey: 'liikevaihto', tyyppi: 'tulo', summa: 1000 }),
        expect.objectContaining({ categoryKey: 'henkilostokulut', tyyppi: 'kulu', summa: 200 }),
        expect.objectContaining({ categoryKey: 'poistot', tyyppi: 'poisto', summa: 50 }),
      ]),
    );
  });
  ...
  it('flips financial type to rahoitus_kulu when combined field is negative', () => {
    ...
    const financial = rows.find((row) => row.categoryKey === 'rahoitustuotot_ja_kulut');
