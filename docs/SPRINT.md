...
| S-23 | Decouple Forecast from VEETI-only baseline semantics and add a structured 20-year investment planning model. See S-23 substeps. | apps/web/src/v2/EnnustePageV2.tsx, apps/web/src/v2/v2.css, apps/web/src/api.ts, apps/web/src/i18n/locales/, apps/api/src/v2/v2.service.ts, apps/api/src/projections/, apps/api/prisma/schema.prisma, apps/api/prisma/migrations/ | Forecast scenarios can be created and explained from trusted effective baseline data; the user can maintain a structured 20-year investment program inside Forecast; existing compute behavior remains explicit and deterministic. | Not eligible (status != READY). Verified evidence so far: `96f53ae` surfaces trusted effective baseline provenance in Forecast; `3302ac2` adds structured investment metadata to the scenario contract and persists it on the existing JSON-backed scenario path without breaking the current editor; `dd18b1a` adds Forecast UI fields for category, type, confidence, and note on each yearly investment row; `d430078` proves the richer rows still merge into projection year overrides using amount-only compute semantics. Summary UI and compatibility tests remain open. | Stop if the new investment model would invalidate existing scenario payloads without a non-destructive fallback path. | IN_PROGRESS |
