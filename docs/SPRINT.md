...
| S-72 | Remove low-value technical helper text and keep raw economic validation primary without inferred badges. See S-72 substeps. | apps/web/src/v2/OverviewPageV2.tsx, apps/web/src/v2/v2.css, apps/web/src/i18n/locales/en.json, apps/web/src/i18n/locales/fi.json, apps/web/src/i18n/locales/sv.json, apps/web/src/v2/OverviewPageV2.test.tsx | Import/review cards no longer lead with secondary technical text like raw row-count hints, the UI does not add “obviously OK” or similar inference badges, and users validate years from visible accounting rows and result values instead. | Evidence needed. | Stop if removing the helper text leaves admins without an accessible path to the same technical detail when they genuinely need it. | TODO |
