...
- [ ] Run a final Finnish Kronoby UI/UX re-audit (`clear -> search -> connect -> explicit import -> review continue -> blocked-year fix/exclude -> baseline -> unlock`) and record the explicit sprint outcome in `docs/SETUP_WIZARD_UIUX_REAUDIT.md`
  ...
  - evidence: HARD BLOCKED: commit:7bc0ebea8430a6aaf94cb35d9faa16ca71de6a95 | run:N/A (manual browser audit) | files:docs/SETUP_WIZARD_UIUX_REAUDIT.md | reason: authenticated `GET /api/v2/overview` and `GET /api/v2/context` returned `500`, so the setup wizard never rendered
