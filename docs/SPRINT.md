...
- [x] Run the focused login + wizard regression bundle for the new entry copy, support rail, chronology, parked-year, search, and row-edit behavior
  ...
  - evidence: packet:779440bebf9724686bd0b6b0b7a67e1d0e2fd5be | run:pnpm --filter ./apps/web test -- src/App.test.tsx src/components/LoginForm.test.tsx src/v2/AppShellV2.test.tsx src/v2/OverviewPageV2.test.tsx src/v2/yearReview.test.ts src/i18n/locales/localeIntegrity.test.ts && pnpm --filter ./apps/api test -- src/v2/v2.service.spec.ts && pnpm --filter ./apps/web typecheck && pnpm --filter ./apps/api typecheck -> PASS | files:docs/SETUP_WIZARD_UIUX_REAUDIT.md | docs:N/A | status: clean
...
- [ ] Re-run a connected-workspace live audit through VEETI connect, year import, five-year selection/recovery, row edit/save, and baseline context, and record whether the queue succeeded or stopped on a blocker in `docs/SETUP_WIZARD_UIUX_REAUDIT.md`
  ...
  - evidence: GATE BLOCKED: local API cannot reach Postgres on `localhost:5432`, so the connected-workspace live audit could not continue past the login entry after the workstation restart; blocker recorded in `docs/SETUP_WIZARD_UIUX_REAUDIT.md`
