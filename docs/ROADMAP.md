...
4. KVA Excel import + Talousarvio baseline are locked (Option A, `KVA totalt` single-source import, historical-only Talousarvio). Ennuste page is functionally complete per `docs/PROJECTION_UX_PLAN.md`, the trust hardening queue `S-31..S-36` is accepted, the initial wizard rollout `S-37..S-42` is landed, and the current execution target is the corrective wizard queue in `docs/SPRINT.md` (`S-43..S-47` today): separate available VEETI years from explicit workspace imports, make step 3 reachable and step progression truthful, remove the stacked legacy surfaces so only one step body mounts at a time, close wizard locale leaks, and prove the fixed Kronoby flow before treating the setup refactor as complete.
