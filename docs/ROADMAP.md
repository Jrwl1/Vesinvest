...
4. KVA Excel import + Talousarvio baseline are locked (Option A, `KVA totalt` single-source import, historical-only Talousarvio). The broader frontend-overhaul queue through `S-86` is accepted, the `Yhteenveto` perfection queue `S-87..S-92` is accepted, and the step-2/QDIS queue `S-93..S-98` is functionally implemented except for its final blocked audit input. The current execution target in `docs/SPRINT.md` is now a selective historical-year repair and investment-plan queue `S-99..S-106`: add user-confirmed KVA workbook overrides on top of VEETI for the six shared financial rows, make 2024 statement-PDF plus workbook merge semantics explicit, and add an operator-friendly `Investointiohjelma` + `Poistosaannot` entry flow at the start of Ennuste using the PTS workbook defaults.
