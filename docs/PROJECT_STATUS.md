...
- `S-28` is `IN_PROGRESS`: Reports now has the refreshed shell, stronger selection/KPI hierarchy, explicit variants, publication-friendly provenance, tighter appendix blocks, and clearer export/loading states; locale alignment and final regression proof still remain.
...
1. Continue execution from `S-28` substep 8 in `docs/SPRINT.md` using `DO` or explicit whole-sprint `RUNSPRINT`.
