...
- [ ] Make clicking the card enter inline whole-card edit mode with one focused active card at a time
  - files: apps/web/src/v2/OverviewPageV2.tsx, apps/web/src/v2/v2.css, apps/web/src/v2/OverviewPageV2.test.tsx
  - run: pnpm --filter ./apps/web test -- src/v2/OverviewPageV2.test.tsx && pnpm --filter ./apps/web typecheck
  - evidence: HARD BLOCKED: current manual year patch contract cannot edit `AineetJaPalvelut`, so the card cannot truthfully support click-to-edit for every visible canon row inside the web-only `S-89` files-scope. Required scope expansion: apps/web/src/api.ts, apps/api/src/v2/dto/manual-year-completion.dto.ts, apps/api/src/v2/v2.service.ts, and matching tests.
