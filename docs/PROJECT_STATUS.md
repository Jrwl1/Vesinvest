...
- `S-30` is `IN_PROGRESS`: keyboard focus, state-surface messaging, shared-shell regression proof, and cleanup are accepted; final verification gates remain.
...
1. Continue execution from `S-30` substep 5 in `docs/SPRINT.md` using `DO` or explicit whole-sprint `RUNSPRINT`.
