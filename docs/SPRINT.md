...
| S-15 | Reduce redundant Forecast/Report API traffic with short-lived GET cache + explicit force refresh path. See S-15 substeps below.   | apps/web/src/api.ts, apps/web/src/v2/EnnustePageV2.tsx, apps/web/src/v2/ReportsPageV2.tsx                                                                          | Repeated list/context/report GETs within cache TTL reuse cached payload; manual refresh and post-mutation list reloads can bypass cache; no behavior regression in list/detail rendering.                                                               | commit `7d29c25`; web typecheck + overview/AppShell tests PASS                       | Stop if cache introduces stale-state bugs that cannot be bypassed with force refresh.                        | READY  |
