...
## ADR-040: The active M0 execution target is a front-end-led visual reset grounded in the current V2 backend contract

**Date:** 2026-03-19
**Decision:** The next execution target is a unified visual-system overhaul across login, Overview, Forecast, and Reports. The redesign is front-end-led by default and must preserve current workflow truth: explicit year approval, source provenance, compute freshness, depreciation availability, investment-program entry, report readiness, and current route/tab semantics. Backend or schema changes are out of scope unless execution proves a minimal current-contract gap that cannot be solved in the frontend alone.
**Context:** The user approved the modern-trust redesign direction and asked for a protocol-compliant implementation plan. Current code and recent history already carry the latest runtime truths that matter for UI planning: explicit Step 3 approval, first-scenario handoff, investment-program start surface, depreciation feature gating/endpoints, and `computedFromUpdatedAt` freshness checks.
**Consequences:**
- The active sprint queue can focus primarily on `apps/web` surfaces and tests.
- The redesign may not hide or weaken the workflow states the backend currently enforces.
- Any backend work during DO must be justified by a real UI truth gap, not by visual ambition alone.
- Clarification (2026-03-24): this ADR preserves workflow truth and current backend contract boundaries; it does not require preserving the old V2 visual language everywhere. Current direction: Yhteenveto keeps VEETI verification-card semantics, while Ennuste may adopt the newer Signal Grid visual system.

Source: planning session 2026-03-19, `docs/client/Spec f�r uppg�rande av en egendomsf�rvaltningsplan f�r 20 �r.docx`, `docs/client/Investeringsplan PTS.xlsx`, `apps/web/src/v2/AppShellV2.tsx`, `apps/web/src/v2/OverviewPageV2.tsx`, `apps/web/src/v2/EnnustePageV2.tsx`, `apps/web/src/v2/ReportsPageV2.tsx`, `apps/api/src/v2/v2.controller.ts`, `apps/api/src/v2/v2.service.ts`, `apps/api/src/v2/dto/update-scenario.dto.ts`
