...
| S-29 | Align shared shell, tokens, responsive behavior, and cross-screen copy once all three refreshed screens are in place. | apps/web/src/v2/AppShellV2.tsx, apps/web/src/v2/v2.css, apps/web/src/i18n/locales/, apps/web/src/App.css | The refreshed screens share a coherent shell and design system, preserve route behavior, and remain readable on desktop and mobile without visual drift between Overview, Forecast, and Reports. | DONE: Acceptance re-verified against `0e5abe2`, `4f654f4`, `8bcec5a`, `bda2d3b`, `99e504d`, and `1590e36`; shared shell alignment, cleanup, and web verification are complete. | Stop if shared styling changes introduce regressions in untouched app areas that require a broader rewrite than the V2 shell. | DONE |
| S-30 | Run UI hardening, accessibility cleanup, regression proof, and final polish across the refreshed V2 screens. | apps/web/src/v2/, apps/web/src/i18n/locales/, e2e/, docs/SPRINT.md, docs/WORKLOG.md | The incremental UI refresh remains type-safe, test-covered, keyboard-usable, and regression-safe across the core Overview -> Forecast -> Reports flow. | DONE: Acceptance verified against `1c02d48`, `819b0b3`, `465176b`, `383c595`, `fec0124`, and `4d0075f`; root lint, typecheck, and test gates passed after fixing locale encoding regressions. | Stop if green verification requires scope outside refreshed V2 screens or changes to forbidden non-UI product behavior. | DONE |
...
- [x] Align header, nav, drawer, and content-width behavior across Overview, Forecast, and Reports without changing current path syncing or account actions
  - files: apps/web/src/v2/AppShellV2.tsx, apps/web/src/v2/v2.css
  - run: pnpm --filter ./apps/web typecheck
  - evidence: commit:4f654f4ba8b255bdcabea253d9516f727405a5e5 | run:pnpm --filter ./apps/web typecheck -> PASS | files:apps/web/src/v2/AppShellV2.tsx,apps/web/src/v2/v2.css | docs:N/A | status: clean

- [x] Unify trust, warning, variant, and provenance badge semantics so the same state looks the same on every refreshed V2 screen
  ...
  - run: pnpm --filter ./apps/web test -- src/v2
  - evidence: commit:8bcec5acfde8db9f434da19255257a62852db6ad | run:pnpm --filter ./apps/web test -- src/v2 -> PASS | files:apps/web/src/v2/EnnustePageV2.tsx,apps/web/src/v2/OverviewPageV2.tsx,apps/web/src/v2/ReportsPageV2.tsx,apps/web/src/v2/v2.css | docs:N/A | status: clean

- [x] Remove one-off screen-specific style hacks that became unnecessary after the three screen refreshes were completed
  - files: apps/web/src/v2/v2.css, apps/web/src/v2/OverviewPageV2.tsx, apps/web/src/v2/EnnustePageV2.tsx, apps/web/src/v2/ReportsPageV2.tsx
  - run: pnpm --filter ./apps/web typecheck
  - evidence: commit:1590e36776b0024d7643348bce0589a68b16718c | run:pnpm --filter ./apps/web typecheck -> PASS | files:apps/web/src/v2/EnnustePageV2.tsx,apps/web/src/v2/OverviewPageV2.tsx,apps/web/src/v2/ReportsPageV2.tsx | docs:N/A | status: clean

- [x] Align cross-screen copy tone, section labels, and repeated empty/error wording in FI/SV/EN so the refreshed V2 shell reads as one product
  ...
  - run: pnpm --filter ./apps/web test -- src/i18n/locales/localeIntegrity.test.ts
  - evidence: commit:bda2d3b6932abc688fb66dc04e393b8adbc2fdef | run:pnpm --filter ./apps/web test -- src/i18n/locales/localeIntegrity.test.ts -> PASS | files:apps/web/src/i18n/locales/en.json,apps/web/src/i18n/locales/fi.json,apps/web/src/i18n/locales/sv.json | docs:N/A | status: clean

- [x] Validate responsive behavior for the shared shell and all refreshed V2 screens, then fix layout regressions in CSS only where possible
  - files: apps/web/src/v2/v2.css, apps/web/src/v2/AppShellV2.tsx, apps/web/src/v2/
  - run: pnpm --filter ./apps/web typecheck
  - evidence: commit:99e504d185c68e0778b45434252f61f006df89e3 | run:pnpm --filter ./apps/web typecheck -> PASS | files:apps/web/src/v2/v2.css | docs:N/A | status: clean

### S-30 substeps

- [x] Review keyboard order, focus states, and interactive affordances across the refreshed V2 shell and fix obvious accessibility regressions
  - files: apps/web/src/v2/, apps/web/src/App.css
  - run: pnpm --filter ./apps/web typecheck
  - evidence: commit:1c02d485f6506d384e0fc7e8503680f5d70ac3ea | run:pnpm --filter ./apps/web typecheck -> PASS | files:apps/web/src/v2/ReportsPageV2.tsx,apps/web/src/v2/v2.css | docs:N/A | status: clean

- [x] Audit loading, empty, success, and error states across Overview, Forecast, and Reports so the refreshed UI still communicates real backend state changes clearly
  - files: apps/web/src/v2/, apps/web/src/i18n/locales/
  - run: pnpm --filter ./apps/web test -- src/v2
  - evidence: commit:819b0b3d1cd3b5bf2fd5f54c86a157a50da876b9 | run:pnpm --filter ./apps/web test -- src/v2 -> PASS | files:apps/web/src/i18n/locales/en.json,apps/web/src/i18n/locales/fi.json,apps/web/src/i18n/locales/sv.json,apps/web/src/v2/EnnustePageV2.tsx,apps/web/src/v2/OverviewPageV2.tsx,apps/web/src/v2/ReportsPageV2.tsx | docs:N/A | status: clean

- [x] Add or tighten targeted regression coverage for the refreshed Overview -> Forecast -> Reports flow where layout or interaction changed materially
  - files: apps/web/src/v2/, e2e/, apps/api/src/v2/
  - run: pnpm --filter ./apps/web test -- src/v2 && pnpm --filter ./apps/api test -- src/v2
  - evidence: commit:465176bd1068b2aa782dfb461b023fd3fc7709d6 | run:pnpm --filter ./apps/web test -- src/v2 && pnpm --filter ./apps/api test -- src/v2 -> PASS | files:apps/web/src/v2/AppShellV2.test.tsx | docs:N/A | status: clean

- [x] Remove dead CSS, obsolete helper branches, and temporary layout code introduced during the incremental refresh
  - files: apps/web/src/v2/, apps/web/src/App.css
  - run: pnpm --filter ./apps/web typecheck
  - evidence: commit:383c59570b9ccfca90eed4f49c83f146b12f0ede | run:pnpm --filter ./apps/web typecheck -> PASS | files:apps/web/src/App.css,apps/web/src/v2/ReportsPageV2.tsx | docs:N/A | status: clean

- [x] Run final web verification for the refreshed UI and confirm the workspace remains type-safe after cleanup
  - files: apps/web/, docs/SPRINT.md, docs/WORKLOG.md
  - run: pnpm --filter ./apps/web typecheck && pnpm --filter ./apps/web test -- src/v2
  - evidence: commit:fec0124ebc843f83c111005da763828b07b74d44 | run:pnpm --filter ./apps/web typecheck && pnpm --filter ./apps/web test -- src/v2 -> PASS | files:apps/web/src/v2/AppShellV2.test.tsx | docs:N/A | status: clean

- [x] Run final root quality gates once the refreshed UI and related regressions are stable
  - files: apps/api/, apps/web/, e2e/, docs/SPRINT.md, docs/WORKLOG.md
  - run: pnpm lint && pnpm typecheck && pnpm test
  - evidence: commit:4d0075f5f0cc98de47df9055e56007cd8d5ea7a8 | run:pnpm lint && pnpm typecheck && pnpm test -> PASS | files:apps/web/src/i18n/locales/fi.json,apps/web/src/i18n/locales/sv.json | docs:N/A | status: clean
