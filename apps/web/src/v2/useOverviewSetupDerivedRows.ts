...
function getEditedFinancialFieldLabel(t: TFunction, sourceField: string): string {
  ...
  if (sourceField === 'PerusmaksuYhteensa') return t('v2Overview.manualFinancialFixedRevenue', 'Fixed revenue total');
  if (sourceField === 'AineetJaPalvelut') return t('v2Overview.previewAccountingMaterialsLabel', 'Materials and services');
  if (sourceField === 'Henkilostokulut') return t('v2Overview.previewAccountingPersonnelLabel', 'Personnel costs');
  if (sourceField === 'Poistot') return t('v2Overview.previewAccountingDepreciationLabel', 'Depreciation');
  if (sourceField === 'LiiketoiminnanMuutKulut') return t('v2Overview.previewAccountingOtherOpexLabel', 'Other operating costs');
  if (sourceField === 'TilikaudenYliJaama') return t('v2Overview.previewAccountingResultLabel', 'Result');
  return sourceField;
}
...
function mergeYearDataSignals(
  ...
  return {
    completeness: {
      ...completeness,
      tilinpaatos: completeness.tilinpaatos === true || yearData.completeness.tilinpaatos === true,
      taksa: completeness.taksa === true || yearData.completeness.taksa === true,
      tariff_revenue:
        completeness.tariff_revenue === false || yearData.completeness.tariff_revenue === false
          ? false
          : true,
      volume_vesi: completeness.volume_vesi === true || yearData.completeness.volume_vesi === true,
      volume_jatevesi:
        completeness.volume_jatevesi === true || yearData.completeness.volume_jatevesi === true,
    },
    baselineReady: yearData.baselineReady ?? yearRow.baselineReady,
    baselineMissingRequirements:
      yearData.baselineMissingRequirements ?? yearRow.baselineMissingRequirements ?? [],
    baselineWarnings: yearData.baselineWarnings ?? yearRow.baselineWarnings ?? [],
    tariffRevenueReason:
      yearData.tariffRevenueReason ??
      (yearData.completeness.tariff_revenue === false ? tariffRevenueReason ?? null : null),
  };
}

function deriveDatasetCounts(yearData: V2ImportYearDataResponse | undefined): Record<string, number> | undefined {
  const counts =
...
export function getExactEditedFieldLabels(params: {
  ...
  const { t, yearData, changedSummaryKeys, manualFinancialFieldSources, statementImportFieldSources, workbookImportFieldSources } = params;
  ...
  if (labels.size === 0) {
    for (const key of changedSummaryKeys) {
      labels.add(buildImportYearSummaryLabel(t, key as never));
    }
  }
...
export function useOverviewSetupDerivedRows(params: {
  ...
  const importBoardRows = React.useMemo(() => {
    return selectableImportYearRows.map((row) => {
      ...
      const usesBaselineReadiness = typeof row.baselineReady === 'boolean' || baselineMissing.size > 0;
      const missingRequiredInputs = [
        {
          present: usesBaselineReadiness ? !baselineMissing.has('financialBaseline') : row.completeness?.tilinpaatos,
          label: t('v2Overview.datasetFinancials', 'Baseline economics'),
        },
        {
          present: usesBaselineReadiness ? !baselineMissing.has('prices') : row.completeness?.taksa,
      ...
      const hasTariffBaselineWarning =
        row.syncBlockedReason == null && row.baselineWarnings?.includes('tariffRevenueMismatch') === true;
      ...
      const incompleteSource =
        row.sourceStatus === 'INCOMPLETE' || trustSignal.reasons.includes('incomplete_source');
      ...
      const isCurrentYearEstimate = row.planningRole === 'current_year_estimate';
        ...
        lane === 'blocked'
          ...
          : hasTariffBaselineWarning
            ...
            : missingCoreCostStructure
              ? t('v2Overview.trustMissingKeyCosts', 'Missing key cost rows')
              : hasLargeDiscrepancy
                ? t('v2Overview.trustLargeDiscrepancy', 'Large discrepancy vs statement')
                : hasFallbackZero
      ...
      const trustToneClass = lane === 'ready' ? 'v2-status-positive' : 'v2-status-warning';
  ...
  }, [defaultBaselineYearSet, excludedYearSet, selectedYears, selectableImportYearRows, yearDataCache, t]);

  const readyTrustBoardRows = React.useMemo(
    () => importBoardRows.filter((row) => isHistoricalPlanningYear(row) && !row.isExcludedFromPlan && row.lane === 'ready'),
    [importBoardRows],
  );
  const suspiciousTrustBoardRows = React.useMemo(
    () => importBoardRows.filter((row) => isHistoricalPlanningYear(row) && !row.isExcludedFromPlan && row.lane === 'suspicious'),
    [importBoardRows],
  );
  const trashbinTrustBoardRows = React.useMemo(
    () => importBoardRows.filter((row) => isHistoricalPlanningYear(row) && row.isExcludedFromPlan),
    [importBoardRows],
  );
  const blockedTrustBoardRows = React.useMemo(
    () => importBoardRows.filter((row) => isHistoricalPlanningYear(row) && !row.isExcludedFromPlan && row.lane === 'blocked'),
    [importBoardRows],
  );
  const currentYearEstimateBoardRows = React.useMemo(
    () => importBoardRows.filter((row) => row.planningRole === 'current_year_estimate'),
    [importBoardRows],
  );
  ...
  const reviewedImportedYearSet = React.useMemo(
    () =>
      new Set(
        [
          ...(reviewedImportedYears.length > 0 ? reviewedImportedYears : persistedReviewedImportedYears),
          ...backendAcceptedPlanningYears,
        ].filter((year) => Number.isFinite(year)),
      ),
    [backendAcceptedPlanningYears, persistedReviewedImportedYears, reviewedImportedYears],
  );
      ...
      [...syncYearRows]
        ...
        .map((row) => {
          ...
          const effectiveRow = {
            ...
            baselineMissingRequirements: mergedSignals.baselineMissingRequirements,
      ...
      importYearRows.filter(
        (row) =>
          getSetupYearStatus(
            {
              ...row,
              reviewState: reviewedImportedYearSet.has(row.vuosi) ? 'reviewed' : 'pending_review',
            },
  ...
  const importReviewStatusRows = React.useMemo(() => {
    ...
    for (const year of excludedYearsSorted) {
      ...
      rows.push({
        ...
        baselineReady: false,
        baselineMissingRequirements: ['financialBaseline', 'prices', 'volumes'] as Array<'financialBaseline' | 'prices' | 'volumes'>,
  ...
  const acceptedPlanningYearRows = React.useMemo(() => {
    const importBoardRowByYear = new Map(importBoardRows.map((row) => [row.vuosi, row]));
    const acceptedBaselineYears = Array.isArray(planningContext?.baselineYears)
      ? planningContext.baselineYears
      : [];
    const importYearRowByYear = new Map(importYearRows.map((row) => [row.vuosi, row]));
      ...
      .map((baselineYear) => {
        ...
        if (importYearRow) {
          return {
            ...
            sourceLayers: buildImportYearSourceLayers(yearDataCache[importYearRow.vuosi]),
          };
        }
        ...
        if (!syncRow) {
          return {
            ...
            warnings: [],
            sourceLayers: buildImportYearSourceLayers(yearDataCache[baselineYear.year]),
          };
        }
      ...
      .map((year) => {
        ...
        if (importYearRow) {
          return {
            ...
            baselineMissingRequirements: importYearRow.baselineMissingRequirements,
            baselineWarnings: importYearRow.baselineWarnings ?? [],
            tariffRevenueReason: importYearRow.tariffRevenueReason ?? null,
  ...
  const reviewStatusRows = React.useMemo(() => {
    ...
    for (const row of acceptedPlanningYearRows) {
      ...
      rows.push({
        ...
        baselineReady: true,
        baselineMissingRequirements: [] as Array<'financialBaseline' | 'prices' | 'volumes'>,
        baselineWarnings: row.baselineWarnings ?? [],
        tariffRevenueReason: row.tariffRevenueReason ?? null,
        readinessChecks: [
          { key: 'financials', labelKey: 'v2Overview.datasetFinancials', ready: row.completeness.tilinpaatos === true },
          { key: 'prices', labelKey: 'v2Overview.datasetPrices', ready: row.completeness.taksa === true },
          { key: 'tariffRevenue', labelKey: 'v2Overview.datasetTariffRevenue', ready: true },
          {
            key: 'volumes',
            labelKey: 'v2Overview.datasetWaterVolume',
            ready: row.completeness.volume_vesi === true || row.completeness.volume_jatevesi === true,
          },
        ],
        missingRequirements: [] as MissingRequirement[],
  ...
  const correctedPlanningManualDataTypes = React.useMemo(
    () => [...new Set(correctedPlanningYearRows.flatMap((row) => row.sourceBreakdown?.manualDataTypes ?? []))].sort(),
    [correctedPlanningYearRows],
  );
  const correctedPlanningVeetiDataTypes = React.useMemo(
    () => [...new Set(correctedPlanningYearRows.flatMap((row) => row.sourceBreakdown?.veetiDataTypes ?? []))].sort(),
    [correctedPlanningYearRows],
  );
